/*
THIS IS NO LONGER THE ORIGINAL FILE, IT HAS BEEN ALTERED!!
IT WILL EVENTUALLY BE REPLACED THOUGH.
...
btCollisionShape *
    gShapePtr[maxNumObjects]; // 1 rigidbody has 1 shape (no re-use of shapes)
...
void DemoApplication::shootBox(const btVector3 &destination) {
  ...
  if (m_dynamicsWorld) {
    ...
    body->setCcdSweptSphereRadius(0.4f); // value should be smaller (embedded)
                                         // than the half extends of the box
...
void DemoApplication::mouseFunc(int button, int state, int x, int y) {
  ...
  case GLFW_MOUSE_BUTTON_RIGHT: {
    if (state == 0) {
      ...
      // apply an impulse
      if (m_dynamicsWorld) {
        btCollisionWorld::ClosestRayResultCallback rayCallback(m_cameraPosition,
                                                               rayTo);
        ...
        if (rayCallback.hasHit()) {
          ...
          if (body) {
            ...
            btVector3 relPos =
                rayCallback.m_hitPointWorld - body->getCenterOfMassPosition();
            ...
            if (!(body->isStaticObject() || body->isKinematicObject())) {
              ...
              if ((m_modifierKeys & GLFW_MOD_SHIFT) == 0) {
                ...
                btGeneric6DofConstraint *dof6 =
                    new btGeneric6DofConstraint(*body, tr, false);
...
btRigidBody *DemoApplication::localCreateRigidBody(
...
void DemoApplication::displayProfileString(int xOffset, int yStart,
                                           char *message) {
    ...
    if (!(getDebugMode() & btIDebugDraw::DBG_DrawWireframe)) {
      ...
      case 1:
        m_shapeDrawer->drawShadow(m, m_sundirection * rot,
                                  colObj->getCollisionShape(), aabbMin,
                                  aabbMax);
        break;
      case 2:
        m_shapeDrawer->drawOpenGL(m, colObj->getCollisionShape(),
                                  wireColor * btScalar(0.3), 0, aabbMin,
                                  aabbMax);
        break;
      }
    }
    ...
    if ((m_debugMode & btIDebugDraw::DBG_NoHelpText) == 0) {
      ...
      if (getDebugMode() & btIDebugDraw::DBG_ProfileTimings) {
        ...
        for (iter = btProfiler::mProfileBlocks.begin();
             iter != btProfiler::mProfileBlocks.end(); ++iter) {
          char blockTime[128];
          sprintf(
              blockTime, "%s: %lf", &((*iter).first[0]),
              btProfiler::getBlockTime(
                  (*iter).first,
  ...
  if (m_dynamicsWorld) {
    ...
    for (i = 0; i < numObjects; i++) {
      ...
      if (body) {
        ...
        // removed cached contact points (this is not necessary if all objects
        // have been removed from the dynamics world)
