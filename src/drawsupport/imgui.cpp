...
// Read 'Programmer guide' below for notes on how to setup ImGui in your
// codebase.
...
the weaknesses includes:
 ...
 - occasionally use statically sized buffers for string manipulations - won't
crash, but some long text may be clipped
 ...
 - click upper right corner to close a window, available when 'bool* p_opened'
...
subtract (because - would set a negative value!)
 ...
 - your code creates the UI, if your code doesn't run the UI is gone! == dynamic
UI, no construction step, less data retention on your side, no state
duplication, less sync, less errors.
 - call and read ImGui::ShowTestWindow() for user-side sample code
 - see examples/ folder for standalone sample applications.
 - customization: use the style editor or PushStyleColor/PushStyleVar to tweak
...
ImGui::GetIO() and fill the 'Input' data, then call ImGui::NewFrame().
      2/ use any ImGui function you want between NewFrame() and Render()
      3/ ImGui::Render() to render all the accumulated command-lists. it will
call your RenderDrawListFn handler that you set in the IO structure.
...
regardless of "update" vs "render" considerations.
...
&bytes_per_pixels);
        ...
        while (true)
        {
            ...
            // 3) most of your application code here - you can use any of
ImGui::* functions at any point in the frame
   ...
   - after calling ImGui::NewFrame() you can read back 'io.WantCaptureMouse' and
'io.WantCaptureKeyboard' to tell if ImGui
     wants to use your inputs. if it does you can discard/hide the inputs from
...
fonts and manage the texture buffer for internal needs.
                       it is now recommended your sample the font texture with
...
(0.5f,0.5f) or (0.375f,0.375f)
 ...
 If you are confused about the meaning or use of ID in ImGui:
   - many widgets requires state to be carried over multiple frames (most
typically ImGui often wants remember what is the "active" widget).
     to do so they need an unique ID. unique ID are typically derived from a
string label, an indice or a pointer.
     when you call Button("OK") the button shows "OK" and also use "OK" as an
ID.
   - ID are uniquely scoped within Windows so no conflict can happen if you have
two buttons called "OK" in two different Windows.
     within a same Window, use PushID() / PopID() to easily create scopes and
avoid ID conflicts.
     so if you have a loop creating "multiple" items, you can use PushID() /
PopID() with the index of each item, or their pointer, etc.
     some functions like TreeNode() implicitly creates a scope for you by
calling PushID()
   - when dealing with trees, ID are important because you want to preserve the
opened/closed state of tree nodes.
     depending on your use cases you may want to use strings, indices or
pointers as ID. experiment and see what makes more sense!
      e.g. When displaying a single object that may change over time, using a
static string as ID will preserve your node open/closed state when the targeted
object change
      e.g. When displaying a list of objects, using indices or pointers as ID
will preserve the node open/closed state per object
   - when passing a label you can optionally specify extra unique ID information
within the same string using "##". This helps solving the simpler collision
cases.
   ...
   - read articles about immediate-mode ui principles (see web links) to
understand the requirement and use of ID.

 If you want to load a different font than the default (ProggyClean.ttf, size
13)

     io.Fonts->AddFontFromFileTTF("myfontfile.ttf", size_in_pixels);
     io.Fonts->GetTexDataAsRGBA32() or GetTexDataAsAlpha8()

 If you want to load multiple fonts, use the font atlas to pack them into a
single texture!
...
size_in_pixels);
 ...
 If you want to display Chinese, Japanese, Korean characters, pass custom
Unicode ranges when loading a font:
...
io.Fonts->GetGlyphRangesJapanese());  // Load Japanese characters
 ...
 If you want to input Japanese/Chinese/Korean in the text input widget:
    ...
    - when loading the font, pass a range that contains the characters you need,
e.g.: io.Fonts->GetGlyphRangesJapanese()
    - to have the Microsoft IME cursor appears at the right location in the
screen, setup a handler for the io.ImeSetInputScreenPosFn function:
...
frame, it will keep appending to the same window.
 ...
 - tip: call and read the ShowTestWindow() code for more example of how to use
ImGui!
 ...
 - misc: merge or clarify ImVec4 / ImGuiAabb, they are essentially duplicate
containers
 - window: add horizontal scroll
 - window: fix resize grip rendering scaling along with Rounding style setting
 - window: autofit feedback loop when user relies on any dynamic layout (window
width multiplier, column). maybe just clearly drop manual autofit?
 - window: add a way for very transient windows (non-saved, temporary overlay
over hundreds of objects) to "clean" up from the global window list.
...
rounding)
 ...
 - widgets: clicking on widget b while widget a should activate widget b
(doesn't anymore because of hover capture)
 - widgets: display mode: widget-label, label-widget (aligned on column or using
fixed size), label-newline-tab-widget etc.
 - widgets: clip text? hover clipped text shows it in a tooltip or in-place
overlay
 - main: considering adding EndFrame()/Init(). some constructs are awkward in
the implementation because of the lack of them.
 - main: IsItemHovered() returns true even if mouse is active on another widget
(e.g. dragging outside of sliders). Maybe not a sensible default? Add parameter
...
TreePop; } if IsHovered' return the hover state of the TreeNode?
 - scrollbar: use relative mouse movement when first-clicking inside of scroll
grab box.
 - scrollbar: make the grab visible and a minimum size for long scroll regions
!- input number: very large int not reliably supported because of int<>float
conversions.
...
ID)
 - file selection widget -> build the tool in our codebase to improve
model-dialog idioms
 - slider: allow using the [-]/[+] buttons used by InputFloat()/InputInt()
 - slider: initial absolute click is imprecise. change to relative movement
slider? hide mouse cursor, allow more precise input using less screen-space.
 - text edit: clean up the mess caused by converting UTF-8 <> wchar. the code is
rather inefficient right now.
 - text edit: centered text for slider as input text so it matches typical
positioning.
 - text edit: flag to disable live update of the user buffer.
 - text edit: field resize behavior - field could stretch when being edited?
hover tooltip shows more text?
 - text edit: add multi-line text edit
 - tree: reformulate OpenNextNode() into SetNextWindowCollapsed() api
 - tree: add treenode/treepush int variants? because (void*) cast from int warns
on some platforms/settings
 - settings: write more decent code to allow saving/loading new fields
 - settings: api for per-tool simple persistent data (bool,int,float) in .ini
file
 ! style: store rounded corners in texture to use 1 quad per corner (filled and
wireframe). so rounding have minor cost.
...
current tree node scope)
 - log: be able to right-click and log a window or tree-node into
tty/file/clipboard / generalized context menu?
...
tabbing
 ...
 - input: rework IO to be able to pass actual events to fix temporal aliasing
issues.
 - input: support track pad style scrolling & slider edit.
 - tooltip: move to fit within screen (e.g. when mouse cursor is right of the
screen).
...
consider replacing active/hovered by offset in HSV space?
 - style editor: color child window height expressed in multiple of line height.
 - optimization/render: use indexed rendering to reduce vertex data cost (for
remote/networked imgui)
...
arrays
 ...
 - optimization: specialize for height based clipping first (assume widgets
never go up + height tests before width tests?)
*/
...
static bool ButtonBehaviour(const ImGuiAabb &bb, const ImGuiID &id,
                            bool *out_hovered, bool *out_held,
                            bool allow_key_modifiers, bool repeat = false,
                            bool pressed_on_click = false);
static void LogText(const ImVec2 &ref_pos, const char *text,
                    const char *text_end = NULL);

static void RenderText(ImVec2 pos, const char *text,
                       const char *text_end = NULL,
                       bool hide_text_after_hash = true);
static void RenderTextWrapped(ImVec2 pos, const char *text,
                              const char *text_end, float wrap_width);
static void RenderTextClipped(ImVec2 pos, const char *text,
                              const char *text_end,
                              const ImVec2 *text_size_if_known,
                              const ImVec2 &clip_max);
static void RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col,
                        bool border = true, float rounding = 0.0f);
static void RenderCollapseTriangle(ImVec2 p_min, bool opened,
...
static const char *ImStristr(const char *haystack, const char *needle,
                             const char *needle_end);
static size_t ImFormatString(char *buf, size_t buf_size, const char *fmt, ...);
static size_t ImFormatStringV(char *buf, size_t buf_size, const char *fmt,
                              va_list args);

// Helpers: Misc
static ImU32 ImCrc32(const void *data, size_t data_size, ImU32 seed);
static bool ImLoadFileToMemory(const char *filename, const char *file_open_mode,
                               void **out_file_data, size_t *out_file_size,
...
// Helpers: UTF-8 <> wchar
static int
ImTextCharToUtf8(char *buf, size_t buf_size,
                 unsigned int in_char); // return output UTF-8 bytes count
static ptrdiff_t
ImTextStrToUtf8(char *buf, size_t buf_size, const ImWchar *in_text,
...
static ptrdiff_t
ImTextStrFromUtf8(ImWchar *buf, size_t buf_size, const char *in_text,
                  const char *in_text_end); // return input UTF-8 bytes count
static int ImTextCountCharsFromUtf8(const char *in_text,
                                    const char *in_text_end); // return number
                                                              // of UTF-8
                                                              // code-points
                                                              // (NOT bytes
                                                              // count)
static int
ImTextCountUtf8BytesFromWchar(const ImWchar *in_text,
...
// Technically we should use the rect pack API for that, but it's just simpler
// to hard-core the positions for now.
// As we start using more of the texture atlas (for rounded corners) we can
// change that.
...
ImGuiStyle::ImGuiStyle() {
  ...
  TouchExtraPadding =
      ImVec2(0, 0); // Expand bounding box for touch-based system where touch
                    // position is not accurate enough (unnecessary for mouse
                    // inputs). Unfortunately we don't sort widgets so priority
                    // on overlap will always be given to the first widget
                    // running. So dont grow this too much!
  AutoFitPadding = ImVec2(
      8, 8); // Extra space after auto-fit (double-clicking on resize grip)
  WindowFillAlphaDefault = 0.70f; // Default alpha of window background, if not
...
// Statically allocated font atlas. This is merely a maneuver to keep
// ImFontAtlas definition at the bottom of the .h file (otherwise it'd be inside
// ImGuiIO)
// Also we wouldn't be able to new() one at this point, before users may define
// IO.MemAllocFn.
...
// in the case the user has implicit cast operators between ImVec2 and its own
// types.
...
struct ImGuiColMod // Color modifier, backup of modified data so we can restore
                   // it
    {
  ImGuiCol Col;
...
struct ImGuiStyleMod // Style modifier, backup of modified data so we can
                     // restore it
    {
  ImGuiStyleVar Var;
...
struct ImGuiAabb // 2D axis aligned bounding-box
  ...
  void Add(const ImVec2 &rhs) {
    Min.x = ImMin(Min.x, rhs.x);
    Min.y = ImMin(Min.y, rhs.y);
    Max.x = ImMax(Max.x, rhs.x);
    Max.y = ImMax(Max.x, rhs.x);
  }
  void Add(const ImGuiAabb &rhs) {
    Min.x = ImMin(Min.x, rhs.Min.x);
    Min.y = ImMin(Min.y, rhs.Min.y);
    Max.x = ImMax(Max.x, rhs.Max.x);
    Max.y = ImMax(Max.y, rhs.Max.y);
  }
  void Expand(const ImVec2 &sz) {
    Min -= sz;
    Max += sz;
  }
...
struct ImGuiDrawContext {
  ...
  ImVector<float> ItemWidth; // 0.0: default, >0.0: width in pixels, <0.0: align
                             // xx pixels to the right of window
  ImVector<float> TextWrapPos;
  ImGuiColorEditMode ColorEditMode;
  ImGuiStorage *StateStorage;
  int OpenNextNode; // FIXME: Reformulate this feature like
                    // SetNextWindowCollapsed() API

  float ColumnsStartX;  // Start position from left of window (increased by
  ...
  ImVector<float> ColumnsOffsetsT; // Columns offset normalized 0.0 (far left)
                                   // -> 1.0 (far right)
...
struct ImGuiTextEditState {
  ...
  ImWchar Text[1024]; // edit buffer, we need to persist but can't guarantee the
                      // persistence of the user-provided buffer. so we copy
                      // into own buffer.
  char InitialText[1024 * 3 + 1]; // backup of end-user buffer at the time of
                                  // focus (in UTF-8, unconverted)
  size_t BufSize; // end-user buffer size, <= 1024 (or increase above)
  float Width;    // widget width
  float ScrollX;
  STB_TexteditState StbState;
  float CursorAnim;
  ImVec2 InputCursorScreenPos; // Cursor position in screen space to be used by
                               // IME callback.
  ...
  void SelectAll() {
    ...
    StbState.select_end = (int)ImStrlenW(Text);
  ...
  // Static functions because they are used to render non-focused instances of a
  // text input box
  static const char *GetTextPointerClippedA(ImFont *font, float font_size,
                                            const char *text, float width,
                                            ImVec2 *out_text_size = NULL);
  static const ImWchar *GetTextPointerClippedW(ImFont *font, float font_size,
                                               const ImWchar *text, float width,
                                               ImVec2 *out_text_size = NULL);
  static void RenderTextScrolledClipped(ImFont *font, float font_size,
                                        const char *text, ImVec2 pos_base,
                                        float width, float scroll_x);
};
...
struct ImGuiState {
  ...
  float FontSize; // (Shortcut) == IO.FontGlobalScale * Font->Scale *
  ...
  bool ActiveIdIsFocusedOnly; // Set only by active widget. Denote focus but no
                              // active interaction.
  ...
  bool LogEnabled;
  FILE *LogFile;
  ImGuiTextBuffer *LogClipboard; // pointer so our GImGui static constructor
...
static ImGuiState *GImGui = &GImDefaultState; // We access everything through
                                              // this pointer. NB: this pointer
                                              // is always assumed to be != NULL
...
struct ImGuiWindow {
  ...
  ImVec2 SizeContentsFit; // Size of contents (extents reach by the drawing
                          // cursor) - may not fit within Size.
  float ScrollY;
  float NextScrollY;
  bool ScrollbarY;
  ...
  int SetWindowPosAllowFlags;       // bit ImGuiSetCondition_*** specify if
                                    // SetWindowPos() call is allowed with this
                                    // particular flag.
  int SetWindowSizeAllowFlags;      // bit ImGuiSetCondition_*** specify if
                                    // SetWindowSize() call is allowed with this
                                    // particular flag.
  int SetWindowCollapsedAllowFlags; // bit ImGuiSetCondition_*** specify if
                                    // SetWindowCollapsed() call is allowed with
                                    // this particular flag.

  ImGuiDrawContext DC;
  ImVector<ImGuiID> IDStack;
  ImVector<ImVec4>
      ClipRectStack;     // Scissoring / clipping rectangle. x1, y1, x2, y2.
...
public:
  ...
  bool
  FocusItemRegister(bool is_active,
  ...
  float TitleBarHeight() const {
    return (Flags & ImGuiWindowFlags_NoTitleBar)
               ? 0
               : FontSize() + GImGui->Style.FramePadding.y * 2.0f;
  }
  ImGuiAabb TitleBarAabb() const {
    return ImGuiAabb(Pos, Pos + ImVec2(SizeFull.x, TitleBarHeight()));
  }
  ImVec2 WindowPadding() const {
    return ((Flags & ImGuiWindowFlags_ChildWindow) &&
            !(Flags & ImGuiWindowFlags_ShowBorders))
               ? ImVec2(0, 0)
               : GImGui->Style.WindowPadding;
  }
...
static inline ImGuiWindow *GetCurrentWindow() {
  ...
  IM_ASSERT(g.CurrentWindow !=
            NULL); // ImGui::NewFrame() hasn't been called yet?
...
// FIXME-OPT: Wasting CPU because all SetInt() are preceeded by GetInt() calls
// so we should have the result from lower_bound already in place.
// However we only use SetInt() on explicit user action (so that's maximum once
// a frame) so the optimisation isn't much needed.
...
ImGuiWindow::ImGuiWindow(const char *name) {
  ...
  SetWindowPosAllowFlags = SetWindowSizeAllowFlags =
      SetWindowCollapsedAllowFlags =
          ImGuiSetCondition_Always | ImGuiSetCondition_FirstUseThisSession |
...
bool ImGuiWindow::FocusItemRegister(bool is_active, bool tab_stop) {
  ...
  if (tab_stop && FocusIdxAllRequestNext == IM_INT_MAX &&
      FocusIdxTabRequestNext == IM_INT_MAX && is_active &&
...
void *ImGui::MemAlloc(size_t sz) { return GImGui->IO.MemAllocFn(sz); }

void ImGui::MemFree(void *ptr) { return GImGui->IO.MemFreeFn(ptr); }
  ...
  for (const char *line_start = file_data; line_start < buf_end;) {
    ...
    if (line_start[0] == '[' && line_end > line_start && line_end[-1] == ']') {
      ...
      ImFormatString(name, IM_ARRAYSIZE(name), "%.*s",
                     line_end - line_start - 2, line_start + 1);
...
void *ImGui::GetInternalState() { return GImGui; }

size_t ImGui::GetInternalStateSize() { return sizeof(ImGuiState); }
...
void ImGui::NewFrame() {
  ...
  for (size_t i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    g.IO.MouseDownTime[i] = g.IO.MouseDown[i]
                                ? (g.IO.MouseDownTime[i] < 0.0f
                                       ? 0.0f
                                       : g.IO.MouseDownTime[i] + g.IO.DeltaTime)
                                : -1.0f;
  ...
  for (size_t i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++)
    g.IO.KeysDownTime[i] = g.IO.KeysDown[i]
                               ? (g.IO.KeysDownTime[i] < 0.0f
                                      ? 0.0f
                                      : g.IO.KeysDownTime[i] + g.IO.DeltaTime)
                               : -1.0f;
  ...
  g.IO.WantCaptureMouse =
      (!mouse_owned_by_application && g.HoveredWindow != NULL) ||
      (g.ActiveId != 0);
  ...
  if (g.HoveredWindow && g.IO.MouseWheel != 0.0f) {
    ...
    if (g.IO.KeyCtrl) {
      ...
      if (!(window->Flags & ImGuiWindowFlags_NoScrollWithMouse)) {
        ...
        window->NextScrollY -=
            g.IO.MouseWheel * window->FontSize() * scroll_lines;
      }
    }
  }
...
// NB: behaviour of ImGui after Shutdown() is not tested/guaranteed at the
...
static void AddWindowToSortedBuffer(ImGuiWindow *window,
  ...
  if (window->Visible) {
    ...
    if (count > 1)
      qsort(window->DC.ChildWindows.begin(), count, sizeof(ImGuiWindow *),
            ChildWindowComparer);
...
static void PushClipRect(const ImVec4 &clip_rect, bool clipped = true) {
  ...
  if (clipped && !window->ClipRectStack.empty()) {
    ...
    cr = ImVec4(ImMax(cr.x, cur_cr.x), ImMax(cr.y, cur_cr.y),
                ImMin(cr.z, cur_cr.z), ImMin(cr.w, cur_cr.w));
  }
...
void ImGui::Render() {
  ...
  if (first_render_of_the_frame) {
    ...
    // Select window for move/focus when we're done with all our widgets (we
    // only consider non-child windows here)
    if (g.ActiveId == 0 && g.HoveredId == 0 && g.HoveredRootWindow != NULL &&
        g.IO.MouseClicked[0])
    ...
    for (size_t i = 0; i != g.Windows.size(); i++) {
      ImGuiWindow *window = g.Windows[i];
      if (window->Flags & ImGuiWindowFlags_ChildWindow) // if a child is visible
                                                        // its parent will add
                                                        // it
        if (window->Visible)
          continue;
    ...
    IM_ASSERT(g.Windows.size() ==
              g.RenderSortedWindows.size()); // we done something wrong
  ...
  // Note that vertex buffers have been created, so it is best practice that you
  // don't call Begin/End in the first place.
  if (g.Style.Alpha > 0.0f) {
    ...
    if (g.IO.MouseDrawCursor) {
      ...
      const ImVec2 tex_uv_scale(1.0f / g.IO.Fonts->TexWidth,
                                1.0f / g.IO.Fonts->TexHeight);
      static ImDrawList draw_list;
      draw_list.Clear();
      draw_list.AddImage(tex_id, pos + ImVec2(1, 0), pos + ImVec2(1, 0) + size,
                         TEX_ATLAS_POS_MOUSE_CURSOR_BLACK * tex_uv_scale,
                         (TEX_ATLAS_POS_MOUSE_CURSOR_BLACK + size) *
                             tex_uv_scale,
                         0x30000000); // Shadow
      draw_list.AddImage(tex_id, pos + ImVec2(2, 0), pos + ImVec2(2, 0) + size,
                         TEX_ATLAS_POS_MOUSE_CURSOR_BLACK * tex_uv_scale,
                         (TEX_ATLAS_POS_MOUSE_CURSOR_BLACK + size) *
                             tex_uv_scale,
                         0x30000000); // Shadow
      ...
      draw_list.AddImage(tex_id, pos, pos + size,
                         TEX_ATLAS_POS_MOUSE_CURSOR_WHITE * tex_uv_scale,
                         (TEX_ATLAS_POS_MOUSE_CURSOR_WHITE + size) *
                             tex_uv_scale,
    ...
    if (!g.RenderDrawLists.empty())
      g.IO.RenderDrawListsFn(&g.RenderDrawLists[0],
...
static const char *FindTextDisplayEnd(const char *text,
  ...
  while ((!text_end || text_display_end < text_end) &&
         *text_display_end != '\0' &&
  ...
  for (;;) {
    ...
    if (line_end != NULL &&
      ...
      if (log_new_line || !is_first_line)
        ImGui::LogText(STR_NEWLINE "%*s%.*s", tree_depth * 4, "", char_count,
                       text_remaining);
      else
...
static float CalcWrapWidthForPos(const ImVec2 &pos, float wrap_pos_x) {
  ...
  const float wrap_width =
      wrap_pos_x > 0.0f ? ImMax(wrap_pos_x - pos.x, 0.00001f) : 0.0f;
  return wrap_width;
}
...
static void RenderText(ImVec2 pos, const char *text, const char *text_end,
  ...
  if (text_len > 0) {
    // Render
    window->DrawList->AddText(window->Font(), window->FontSize(), pos,
                              window->Color(ImGuiCol_Text), text,
                              text_display_end);
...
static void RenderTextWrapped(ImVec2 pos, const char *text,
  ...
  if (text_len > 0) {
    // Render
    window->DrawList->AddText(window->Font(), window->FontSize(), pos,
                              window->Color(ImGuiCol_Text), text, text_end,
                              wrap_width);
...
static void RenderTextClipped(ImVec2 pos, const char *text,
  ...
  if (text_len > 0) {
    ...
    // scissor state
    const bool need_clipping = (pos.x + text_size.x >= clip_max.x) ||
                               (pos.y + text_size.y >= clip_max.y);

    // Render
    window->DrawList->AddText(
        window->Font(), window->FontSize(), pos, window->Color(ImGuiCol_Text),
        text, text_display_end, 0.0f, need_clipping ? &clip_max : NULL);
...
static void RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border,
                        float rounding) {
  ImGuiWindow *window = GetCurrentWindow();

  window->DrawList->AddRectFilled(p_min, p_max, fill_col, rounding);
  if (border && (window->Flags & ImGuiWindowFlags_ShowBorders)) {
    // FIXME: This is the best I've found that works on multiple renderer/back
    // ends. Bit dodgy.
    window->DrawList->AddRect(p_min + ImVec2(1.5f, 1.5f), p_max + ImVec2(1, 1),
                              window->Color(ImGuiCol_BorderShadow), rounding);
    window->DrawList->AddRect(p_min + ImVec2(0.5f, 0.5f), p_max + ImVec2(0, 0),
                              window->Color(ImGuiCol_Border), rounding);
  }
}

// Render a triangle to denote expanded/collapsed state
static void RenderCollapseTriangle(ImVec2 p_min, bool opened, float scale,
                                   bool shadow) {
  ImGuiWindow *window = GetCurrentWindow();
...
ImVec2 ImGui::CalcTextSize(const char *text, const char *text_end,
  ...
  if (hide_text_after_double_hash)
    text_display_end =
        FindTextDisplayEnd(text, text_end); // Hide anything after a '##' string
  else
    text_display_end = text_end;
...
// If you are displaying thousands of items and you have a random access to the
// list, you can perform clipping yourself to save on CPU.
// {
//    float item_height = ImGui::GetTextLineHeightWithSpacing();
//    int display_start, display_end;
//    ImGui::CalcListClipping(count, item_height, &display_start, &display_end);
//    // calculate how many to clip/display
//    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (display_start) *
//    item_height);         // advance cursor
//    for (int i = display_start; i < display_end; i++)
//    // display only visible items
//        // TODO: display visible item
//    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (count - display_end) *
//    item_height);   // advance cursor
// }
...
static ImGuiWindow *FindHoveredWindow(ImVec2 pos, bool excluding_childs) {
  ...
  for (int i = (int)g.Windows.size() - 1; i >= 0; i--) {
    ...
    // parent.
    ImGuiAabb bb(window->ClippedAabb.Min - g.Style.TouchExtraPadding,
                 window->ClippedAabb.Max + g.Style.TouchExtraPadding);
    if (bb.Contains(pos))
      return window;
  }
  return NULL;
...
// NB- Expand the aabb to be generous on imprecise inputs systems
...
static bool IsMouseHoveringBox(const ImGuiAabb &bb) {
  ...
  if (!window->ClipRectStack.empty()) {
    const ImVec4 clip_rect = window->ClipRectStack.back();
    box_clipped.Clip(ImGuiAabb(ImVec2(clip_rect.x, clip_rect.y),
                               ImVec2(clip_rect.z, clip_rect.w)));
  }

  // Expand for touch input
  const ImGuiAabb box_for_touch(box_clipped.Min - g.Style.TouchExtraPadding,
                                box_clipped.Max + g.Style.TouchExtraPadding);
...
bool ImGui::IsKeyPressed(int key_index, bool repeat) {
  ...
  if (repeat && t > KEY_REPEAT_DELAY)
    if ((fmodf(t - KEY_REPEAT_DELAY, KEY_REPEAT_RATE) >
         KEY_REPEAT_RATE * 0.5f) !=
        (fmodf(t - KEY_REPEAT_DELAY - g.IO.DeltaTime, KEY_REPEAT_RATE) >
         KEY_REPEAT_RATE * 0.5f))
      return true;
...
bool ImGui::IsMouseClicked(int button, bool repeat) {
  ...
  if (repeat && t > MOUSE_REPEAT_DELAY)
    if ((fmodf(t - MOUSE_REPEAT_DELAY, MOUSE_REPEAT_RATE) >
         MOUSE_REPEAT_RATE * 0.5f) !=
        (fmodf(t - MOUSE_REPEAT_DELAY - g.IO.DeltaTime, MOUSE_REPEAT_RATE) >
         MOUSE_REPEAT_RATE * 0.5f))
      return true;
...
void ImGui::BeginTooltip() {
  ...
  ImGui::Begin("##Tooltip", NULL, ImVec2(0, 0),
               g.Style.Colors[ImGuiCol_TooltipBg].w, window_flags);
}
...
void ImGui::EndChild() {
  ...
  if (window->Flags & ImGuiWindowFlags_ComboBox) {
    ...
    // When using auto-filling child window, we don't provide the width/height
...
// - 'size' for a regular window denote the initial size for first-time creation
// (no saved data) and isn't that useful. Use SetNextWindowSize() prior to
// calling Begin() for more flexible window manipulation.
...
// file). Note that you can use ## to append unique data that isn't displayed,
// e.g. "My window##1" will use "My window##1" as unique window ID but display
// "My window" to the user.
...
// - Passing 'bool* p_opened' displays a Close button on the upper-right corner
...
// SetNextWindowSize(size, ImGuiSetCondition_FirstUseEver) prior to calling
// Begin().
bool ImGui::Begin(const char *name, bool *p_opened, const ImVec2 &size,
  ...
  if (g.SetNextWindowCollapsedCond) {
    ImGui::SetWindowCollapsed(g.SetNextWindowCollapsedVal,
  ...
  while (root_idx > 0) {
    if ((g.CurrentWindowStack[root_idx]->Flags &
         ImGuiWindowFlags_ChildWindow) == 0)
      break;
    root_idx--;
  }
  ...
  if ((flags & ImGuiWindowFlags_ChildWindow) &&
      !(flags & ImGuiWindowFlags_ComboBox))
    PushClipRect(parent_window->ClipRectStack.back());
  else if (g.IO.DisplayVisibleMin.x != g.IO.DisplayVisibleMax.x &&
           g.IO.DisplayVisibleMin.y != g.IO.DisplayVisibleMax.y)
    PushClipRect(ImVec4(g.IO.DisplayVisibleMin.x, g.IO.DisplayVisibleMin.y,
                        g.IO.DisplayVisibleMax.x, g.IO.DisplayVisibleMax.y));
  else
  ...
  if (first_begin_of_the_frame) {
    ...
    // Move window (at the beginning of the frame to avoid input lag or
    // sheering)
    ...
    if (!window_pos_set_by_api &&
      ...
      window->PosFloat =
          g.IO.MousePos + ImVec2(32, 16) - style.FramePadding * 2;
    }
    ...
    if (!(window->Flags & ImGuiWindowFlags_ChildWindow) &&
      ...
      const ImVec2 pad =
          ImVec2(window->FontSize() * 2.0f,
                 window->FontSize() *
                     2.0f); // FIXME: Parametrize of clarify this behavior.
    ...
    if (window->FocusIdxAllRequestNext == IM_INT_MAX ||
      ...
      window->FocusIdxAllRequestCurrent =
          ...
          (window->FocusIdxAllCounter + 1);
    if (window->FocusIdxTabRequestNext == IM_INT_MAX ||
    ...
    const float window_rounding = (window->Flags & ImGuiWindowFlags_ChildWindow)
    ...
    if (window->Collapsed) {
      ...
      window->DrawList->AddRectFilled(
          title_bar_aabb.GetTL(), title_bar_aabb.GetBR(),
          window->Color(ImGuiCol_TitleBgCollapsed), window_rounding);
      if (window->Flags & ImGuiWindowFlags_ShowBorders) {
        window->DrawList->AddRect(title_bar_aabb.GetTL() + ImVec2(1, 1),
                                  title_bar_aabb.GetBR() + ImVec2(1, 1),
                                  window->Color(ImGuiCol_BorderShadow),
                                  window_rounding);
      ...
      if ((window->Flags & ImGuiWindowFlags_Tooltip) != 0) {
        ...
        const ImVec2 size_auto_fit = window->SizeContentsFit +
                                     style.WindowPadding -
                                     ImVec2(0.0f, style.ItemSpacing.y);
        window->SizeFull = size_auto_fit;
      } else {
        const ImVec2 size_auto_fit = ImClamp(
            window->SizeContentsFit + style.AutoFitPadding, style.WindowMinSize,
            ImMax(style.WindowMinSize,
                  g.IO.DisplaySize - style.AutoFitPadding));
        if ((window->Flags & ImGuiWindowFlags_AlwaysAutoResize) != 0) {
          // Don't continuously mark settings as dirty, the size of the window
          ...
          // Manual resize grip
          const ImGuiAabb resize_aabb(window->Aabb().GetBR() - ImVec2(18, 18),
                                      window->Aabb().GetBR());
        ...
        // Update aabb immediately so that rendering right below us isn't one
        // frame late
        title_bar_aabb = window->TitleBarAabb();
      }

      // Scrollbar
      window->ScrollbarY = (window->SizeContentsFit.y > window->Size.y) &&
      ...
      if (bg_alpha > 0.0f) {
        if ((window->Flags & ImGuiWindowFlags_ComboBox) != 0)
          window->DrawList->AddRectFilled(
              ...
              window->Color(ImGuiCol_ComboBg, bg_alpha), window_rounding);
        else if ((window->Flags & ImGuiWindowFlags_Tooltip) != 0)
          window->DrawList->AddRectFilled(
              window->Pos, window->Pos + window->Size,
              window->Color(ImGuiCol_TooltipBg, bg_alpha), window_rounding);
        else if ((window->Flags & ImGuiWindowFlags_ChildWindow) != 0)
          window->DrawList->AddRectFilled(
              window->Pos,
              window->Pos + window->Size -
                  ImVec2(window->ScrollbarY ? style.ScrollBarWidth : 0.0f,
                         0.0f),
              window->Color(ImGuiCol_ChildWindowBg, bg_alpha), window_rounding,
              window->ScrollbarY ? (1 | 8) : (0xF));
        else
          window->DrawList->AddRectFilled(
              window->Pos, window->Pos + window->Size,
              window->Color(ImGuiCol_WindowBg, bg_alpha), window_rounding);
      }
      ...
      if (!(window->Flags & ImGuiWindowFlags_NoTitleBar))
        window->DrawList->AddRectFilled(
            title_bar_aabb.GetTL(), title_bar_aabb.GetBR(),
            window->Color(ImGuiCol_TitleBg), window_rounding, 1 | 2);
      ...
      if (window->Flags & ImGuiWindowFlags_ShowBorders) {
        ...
        window->DrawList->AddRect(window->Pos, window->Pos + window->Size,
                                  ...
                                  window_rounding);
        if (!(window->Flags & ImGuiWindowFlags_NoTitleBar))
          window->DrawList->AddLine(title_bar_aabb.GetBL(),
                                    title_bar_aabb.GetBR(),
                                    window->Color(ImGuiCol_Border));
      }

      // Scrollbar
      if (window->ScrollbarY) {
        ImGuiAabb scrollbar_bb(window->Aabb().Max.x - style.ScrollBarWidth,
                               title_bar_aabb.Max.y + 1, window->Aabb().Max.x,
                               window->Aabb().Max.y - 1);
        // window->DrawList->AddLine(scrollbar_bb.GetTL(), scrollbar_bb.GetBL(),
        // g.Colors[ImGuiCol_Border]);
        ...
        // Handle input right away (none of the code above is relying on
        // scrolling position)
        bool held = false;
        bool hovered = false;
        if (grab_size_y_norm < 1.0f) {
          ...
          if (held) {
            ...
            const float pos_y_norm =
                ImSaturate((g.IO.MousePos.y -
                            (scrollbar_bb.Min.y + grab_size_y * 0.5f)) /
                           (scrollbar_bb.GetHeight() - grab_size_y)) *
                (1.0f - grab_size_y_norm);
            window->ScrollY =
                (float)(int)(pos_y_norm * window->SizeContentsFit.y);
            window->NextScrollY = window->ScrollY;
          }
        }
        ...
        const float pos_y_norm = ImSaturate(
            window->ScrollY / ImMax(0.0f, window->SizeContentsFit.y));
        const ImU32 grab_col =
            window->Color(held ? ImGuiCol_ScrollbarGrabActive
                               : hovered ? ImGuiCol_ScrollbarGrabHovered
                                         : ImGuiCol_ScrollbarGrab);
        window->DrawList->AddRectFilled(
            ImVec2(scrollbar_bb.Min.x,
                   ImLerp(scrollbar_bb.Min.y, scrollbar_bb.Max.y, pos_y_norm)),
            ImVec2(scrollbar_bb.Max.x,
                   ImLerp(scrollbar_bb.Min.y, scrollbar_bb.Max.y,
                          pos_y_norm + grab_size_y_norm)),
            grab_col);
      }
      ...
      if (!(window->Flags & ImGuiWindowFlags_NoResize)) {
        ...
        if (r == 0.0f) {
          window->DrawList->AddTriangleFilled(br, br - ImVec2(0, 14),
                                              br - ImVec2(14, 0), resize_col);
        } else {
          // FIXME: We should draw 4 triangles and decide on a size that's not
          // dependent on the rounding size (previously used 18)
          window->DrawList->AddArc(br - ImVec2(r, r), r, resize_col, 6, 9,
                                   true);
          window->DrawList->AddTriangleFilled(br + ImVec2(0, -2 * r),
                                              br + ImVec2(0, -r),
                                              br + ImVec2(-r, -r), resize_col);
          window->DrawList->AddTriangleFilled(
              br + ImVec2(-r, -r), br + ImVec2(-r, 0), br + ImVec2(-2 * r, 0),
              resize_col);
        }
      }
    }
    ...
    window->DC.CursorStartPos =
        window->Pos +
        ImVec2(window->DC.ColumnsStartX + window->DC.ColumnsOffsetX,
    ...
    if (!(window->Flags & ImGuiWindowFlags_NoTitleBar)) {
      ...
      const ImVec2 text_max =
          window->Pos +
          ImVec2(window->Size.x - (p_opened ? (title_bar_aabb.GetHeight() - 3)
  ...
  ImVec4 clip_rect(
      title_bar_aabb.Min.x + 0.5f + window->WindowPadding().x * 0.5f,
      title_bar_aabb.Max.y + 0.5f,
      window->Aabb().Max.x + 0.5f - window->WindowPadding().x * 0.5f,
  ...
  if (flags & ImGuiWindowFlags_ChildWindow) {
    ...
    window->Collapsed |=
        (clip_rect_t.x >= clip_rect_t.z || clip_rect_t.y >= clip_rect_t.w);
  ...
  window->SkipItems =
      window->Collapsed || (!window->Visible && window->AutoFitFrames == 0);
...
void ImGui::End() {
  ...
  // Pop
  // NB: we don't clear 'window->RootWindow' yet, it will be used then cleared
  // in NewFrame()
...
void ImGui::PushItemWidth(float item_width) {
  ImGuiWindow *window = GetCurrentWindow();
  window->DC.ItemWidth.push_back(item_width == 0.0f ? window->ItemWidthDefault
                                                    : item_width);
}
...
float ImGui::CalcItemWidth() {
  ...
  if (w < 0.0f) {
    ...
    float width_to_right_edge =
        window->Pos.x + ImGui::GetContentRegionMax().x - window->DC.CursorPos.x;
...
static float *GetStyleVarFloatAddr(ImGuiStyleVar idx) {
  ...
  case ImGuiStyleVar_TreeNodeSpacing:
    return &g.Style.TreeNodeSpacing;
  }
  return NULL;
}
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, const ImVec2 &val) {
  ...
  IM_ASSERT(
      pvar !=
      NULL); // Called function with wrong-type? Varialble is not a ImVec2.
  ImGuiStyleMod backup;
  backup.Var = idx;
...
// NB: internally we store CursorPos in absolute screen coordinates because it
// is more convenient.
// Conversion happens as we pass the value to user, but it makes our naming
// convention dodgy. May want to rename 'DC.CursorPos'.
ImVec2 ImGui::GetCursorPos() {
  ImGuiWindow *window = GetCurrentWindow();
  return window->DC.CursorPos - window->Pos;
}

float ImGui::GetCursorPosX() { return ImGui::GetCursorPos().x; }

float ImGui::GetCursorPosY() { return ImGui::GetCursorPos().y; }

void ImGui::SetCursorPos(const ImVec2 &pos) {
  ImGuiWindow *window = GetCurrentWindow();
  window->DC.CursorPos = window->Pos + pos;
  window->SizeContentsFit =
      ImMax(window->SizeContentsFit, pos + ImVec2(0.0f, window->ScrollY));
}
...
void ImGui::SetCursorPosY(float y) {
  ...
  window->SizeContentsFit.y =
      ImMax(window->SizeContentsFit.y, y + window->ScrollY);
}
...
void ImGui::SetScrollPosHere() {
  ImGuiWindow *window = GetCurrentWindow();
  window->NextScrollY = (window->DC.CursorPos.y + window->ScrollY) -
                        (window->Pos.y + window->SizeFull.y * 0.5f) -
                        (window->TitleBarHeight() + window->WindowPadding().y);
}
...
void ImGui::TextV(const char *fmt, va_list args) {
  ...
  const char *text_end =
      buf + ImFormatStringV(buf, IM_ARRAYSIZE(buf), fmt, args);
...
void ImGui::TextUnformatted(const char *text, const char *text_end) {
  ...
  if (text_end - text > 2000 && !wrap_enabled) {
    ...
    if (start_pos.y <= clip_rect.w) {
      ...
      if (!g.LogEnabled) {
        int lines_skippable =
            (int)((clip_rect.y - start_pos.y) / line_height) - 1;
      ...
      // Lines to render
      if (line < text_end) {
        ImGuiAabb line_box(pos,
                           pos + ImVec2(ImGui::GetWindowWidth(), line_height));
        while (line < text_end) {
          const char *line_end = strchr(line, '\n');
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
  ...
  const char *value_text_end =
      value_text_begin + ImFormatStringV(buf, IM_ARRAYSIZE(buf), fmt, args);

  const ImVec2 text_size = CalcTextSize(label, NULL, true);
  const ImGuiAabb value_bb(
      window->DC.CursorPos,
  ...
  const ImGuiAabb bb(
      ...
      window->DC.CursorPos +
          ImVec2(w + style.FramePadding.x * 2 +
                     (text_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f),
                 0.0f) +
          text_size);
  ItemSize(bb);
  ...
  // Render
  RenderTextClipped(value_bb.Min, value_text_begin, value_text_end, NULL,
                    value_bb.Max);
  RenderText(ImVec2(value_bb.Max.x + style.ItemInnerSpacing.x, value_bb.Min.y),
             label);
}
...
static bool IsHovered(const ImGuiAabb &bb, const ImGuiID &id) {
  ...
  if (g.HoveredId == 0) {
    ...
    if (g.HoveredRootWindow == window->RootWindow) {
      bool hovered =
          (g.ActiveId == 0 || g.ActiveId == id || g.ActiveIdIsFocusedOnly) &&
...
static bool ButtonBehaviour(const ImGuiAabb &bb, const ImGuiID &id,
                            bool *out_hovered, bool *out_held,
                            bool allow_key_modifiers, bool repeat,
...
bool ImGui::Button(const char *label, const ImVec2 &size_arg,
  ...
  const ImVec2 size(size_arg.x != 0.0f ? size_arg.x : text_size.x,
                    size_arg.y != 0.0f ? size_arg.y : text_size.y);
  ...
  bool hovered, held;
  bool pressed =
      ButtonBehaviour(bb, id, &hovered, &held, true, repeat_when_held);
  ...
  const ImVec2 off = ImVec2(ImMax(0.0f, size.x - text_size.x) * 0.5f,
                            ImMax(0.0f, size.y - text_size.y) *
                                0.5f); // Center (only applies if we explicitly
                                       // gave a size bigger than the text size,
                                       // which isn't the common path)
  RenderTextClipped(bb.Min + style.FramePadding + off, label, NULL, &text_size,
                    bb.Max); // Render clip (only applies if we explicitly gave
                             // a size smaller than the text size, which isn't
                             // the commmon path)
...
bool ImGui::SmallButton(const char *label) {
  ...
  const ImGuiAabb bb(window->DC.CursorPos,
                     window->DC.CursorPos + text_size +
                         ImVec2(style.FramePadding.x * 2, 0));
  ItemSize(bb);
  if (!ItemAdd(bb, &id))
    return false;
...
static bool CloseWindowButton(bool *p_opened) {
  ...
  const ImGuiAabb bb(window->Aabb().GetTR() + ImVec2(-3.0f - size, 2.0f),
                     window->Aabb().GetTR() + ImVec2(-3.0f, 2.0f + size));
  ...
  // Render
  const ImU32 col =
      window->Color((held && hovered) ? ImGuiCol_CloseButtonActive
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2 &size,
  ...
  const ImGuiAabb image_bb(window->DC.CursorPos + padding,
...
bool ImGui::CollapsingHeader(const char *label, const char *str_id,
  ...
  const ImGuiAabb text_bb(
      bb.Min, bb.Min +
                  ImVec2(window->FontSize() + style.FramePadding.x * 2 * 2, 0) +
                  text_size);
  ItemSize(ImVec2(text_bb.GetSize().x, bb.GetSize().y)); // NB: we don't provide
                                                         // our width so that it
                                                         // doesn't get feed
                                                         // back into AutoFit

  // When logging is enabled, if automatically expand tree nodes (but *NOT*
  // collapsing headers.. seems like sensible behaviour).
  ...
  bool hovered, held;
  bool pressed =
      ButtonBehaviour(display_frame ? bb : text_bb, id, &hovered, &held, false);
  if (pressed) {
    opened = !opened;
    storage->SetInt(id, opened);
  }
  ...
  if (display_frame) {
    ...
    RenderText(bb.Min + style.FramePadding +
                   ImVec2(window->FontSize() + style.FramePadding.x * 2, 0),
               label);
    if (g.LogEnabled) {
    ...
    RenderCollapseTriangle(
        bb.Min + ImVec2(style.FramePadding.x, window->FontSize() * 0.15f),
        opened, 0.70f, false);
    if (g.LogEnabled)
      LogText(bb.Min, ">");
    RenderText(bb.Min +
...
void ImGui::Bullet() {
  ...
  const ImGuiAabb bb(window->DC.CursorPos,
                     window->DC.CursorPos + ImVec2(line_height, line_height));
  ItemSize(bb);
  if (!ItemAdd(bb, NULL))
    return;
  ...
  window->DrawList->AddCircleFilled(
      bb.Min +
          ImVec2(style.FramePadding.x + line_height * 0.5f, line_height * 0.5f),
...
void ImGui::BulletTextV(const char *fmt, va_list args) {
  ...
  const char *text_end =
      text_begin + ImFormatStringV(buf, IM_ARRAYSIZE(buf), fmt, args);
  ...
  const ImGuiAabb bb(
      ...
      window->DC.CursorPos +
          ImVec2(line_height +
                     (text_size.x > 0.0f ? (style.FramePadding.x * 2) : 0.0f),
                 0) +
          text_size); // Empty text doesn't add padding
  ItemSize(bb);
  if (!ItemAdd(bb, NULL))
    return;
  ...
  window->DrawList->AddCircleFilled(
      bb.Min +
          ImVec2(style.FramePadding.x + line_height * 0.5f, line_height * 0.5f),
      bullet_size, window->Color(ImGuiCol_Text));
  RenderText(bb.Min + ImVec2(window->FontSize() + style.FramePadding.x * 2, 0),
             text_begin, text_end);
}
...
// If returning 'true' the node is open and the user is responsible for calling
// TreePop
...
// NB: only call right after InputText because we are using its InitialValue
// storage
...
bool ImGui::SliderFloat(const char *label, float *v, float v_min, float v_max,
  ...
  const ImVec2 text_size = CalcTextSize(label, NULL, true);
  const ImGuiAabb frame_bb(window->DC.CursorPos, window->DC.CursorPos +
                                                     ImVec2(w, text_size.y) +
                                                     style.FramePadding * 2.0f);
  const ImGuiAabb slider_bb(frame_bb.Min + style.FramePadding,
                            frame_bb.Max - style.FramePadding);
  const ImGuiAabb bb(frame_bb.Min,
                     frame_bb.Max +
  ...
  // NB- we don't call ItemSize() yet becausae we may turn into a text edit box
  ...
  const bool is_unbound = v_min == -FLT_MAX || v_min == FLT_MAX ||
                          v_max == -FLT_MAX || v_max == FLT_MAX;

  const float grab_size_in_units = 1.0f; // In 'v' units. Probably needs to be
                                         // parametrized, based on a 'v_step'
  ...
  if (decimal_precision > 0 || is_unbound)
    ...
    grab_size_in_pixels =
        ImMax(grab_size_in_units * (w / (v_max - v_min + 1.0f)),
              8.0f); // Integer sliders
  const float slider_effective_w = slider_bb.GetWidth() - grab_size_in_pixels;
  const float slider_effective_x1 =
      slider_bb.Min.x + grab_size_in_pixels * 0.5f;
  const float slider_effective_x2 =
  ...
  if (start_text_input || (g.ActiveId == id && id == g.SliderAsInputTextId)) {
    char text_buf[64];
    ImFormatString(text_buf, IM_ARRAYSIZE(text_buf), "%.*f", decimal_precision,
                   *v);

    SetActiveId(g.SliderAsInputTextId);
    g.HoveredId = 0;
    window->FocusItemUnregister(); // Our replacement slider will override the
                                   // focus ID (registered previously to allow
                                   // for a TAB focus to happen)
    value_changed = ImGui::InputText(label, text_buf, IM_ARRAYSIZE(text_buf),
  ...
  if (g.ActiveId == id) {
    if (g.IO.MouseDown[0]) {
      if (!is_unbound) {
        const float normalized_pos = ImClamp(
            (g.IO.MousePos.x - slider_effective_x1) / slider_effective_w, 0.0f,
            1.0f);
  ...
  if (!is_unbound) {
    // Linear slider
    // const float grab_t = (ImClamp(*v, v_min, v_max) - v_min) / (v_max -
    // v_min);
    ...
    window->DrawList->AddRectFilled(
        ...
        window->Color(g.ActiveId == id ? ImGuiCol_SliderGrabActive
                                       : ImGuiCol_SliderGrab));
  }

  // Draw value using user-provided display format so user can add
  ...
  RenderTextClipped(
      ImVec2(ImMax(frame_bb.Min.x + style.FramePadding.x,
                   slider_bb.GetCenter().x - value_text_size.x * 0.5f),
             frame_bb.Min.y + style.FramePadding.y),
      value_buf, value_buf_end, &value_text_size, frame_bb.Max);

  RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, slider_bb.Min.y),
             label);

  return value_changed;
}

bool ImGui::SliderAngle(const char *label, float *v, float v_degrees_min,
                        float v_degrees_max) {
  float v_deg = *v * 360.0f / (2 * PI);
  bool value_changed = ImGui::SliderFloat(label, &v_deg, v_degrees_min,
...
bool ImGui::SliderInt(const char *label, int *v, int v_min, int v_max,
  ...
  if (!display_format)
    display_format = "%.0f";
  float v_f = (float)*v;
  bool value_changed = ImGui::SliderFloat(label, &v_f, (float)v_min,
...
static bool SliderFloatN(const char *label, float v[3], int components,
  ...
  for (int i = 0; i < components; i++) {
    ...
    value_changed |=
        ImGui::SliderFloat("##v", &v[i], v_min, v_max, display_format, power);
...
static bool SliderIntN(const char *label, int v[3], int components, int v_min,
  ...
  for (int i = 0; i < components; i++) {
    ...
    value_changed |=
        ImGui::SliderInt("##v", &v[i], v_min, v_max, display_format);
    ImGui::SameLine(0, (int)style.ItemInnerSpacing.x);
    ImGui::PopID();
  }
  ImGui::PopItemWidth();
...
static void Plot(ImGuiPlotType plot_type, const char *label,
  ...
  const ImGuiAabb frame_bb(window->DC.CursorPos,
                           window->DC.CursorPos +
                               ...
                               style.FramePadding * 2.0f);
  const ImGuiAabb graph_bb(frame_bb.Min + style.FramePadding,
                           frame_bb.Max - style.FramePadding);
  const ImGuiAabb bb(frame_bb.Min,
                     frame_bb.Max +
                         ImVec2(text_size.x > 0.0f
  ...
  if (IsMouseHoveringBox(graph_bb)) {
    const float t = ImClamp((g.IO.MousePos.x - graph_bb.Min.x) /
                                (graph_bb.Max.x - graph_bb.Min.x),
                            0.0f, 0.9999f);
    const int v_idx =
        (int)(t *
  ...
  const ImU32 col_base = window->Color((plot_type == ImGuiPlotType_Lines)
                                           ? ImGuiCol_PlotLines
                                           : ImGuiCol_PlotHistogram);
  const ImU32 col_hovered = window->Color((plot_type == ImGuiPlotType_Lines)
  ...
  for (int n = 0; n < res_w; n++) {
    ...
    if (plot_type == ImGuiPlotType_Lines)
      window->DrawList->AddLine(ImLerp(graph_bb.Min, graph_bb.Max, p0),
                                ImLerp(graph_bb.Min, graph_bb.Max, p1),
                                v_hovered == v_idx ? col_hovered : col_base);
    else if (plot_type == ImGuiPlotType_Histogram)
      window->DrawList->AddRectFilled(
          ImLerp(graph_bb.Min, graph_bb.Max, p0),
          ImLerp(graph_bb.Min, graph_bb.Max, ImVec2(p1.x, 1.0f)) +
              ImVec2(-1, 0),
          v_hovered == v_idx ? col_hovered : col_base);
  ...
  if (overlay_text)
    RenderText(
        ImVec2(graph_bb.GetCenter().x -
                   ImGui::CalcTextSize(overlay_text, NULL, true).x * 0.5f,
               frame_bb.Min.y + style.FramePadding.y),
        overlay_text);

  RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, graph_bb.Min.y),
             label);
}
...
struct ImGuiPlotArrayGetterData {
  ...
  ImGuiPlotArrayGetterData(const float *values, size_t stride) {
    Values = values;
    Stride = stride;
  }
};
...
void ImGui::PlotLines(const char *label, const float *values, int values_count,
  ...
  Plot(ImGuiPlotType_Lines, label, &Plot_ArrayGetter, (void *)&data,
...
void ImGui::PlotLines(const char *label,
  ...
  Plot(ImGuiPlotType_Lines, label, values_getter, data, values_count,
...
void ImGui::PlotHistogram(const char *label, const float *values,
                          int values_count, int values_offset,
                          const char *overlay_text, float scale_min,
                          float scale_max, ImVec2 graph_size, size_t stride) {
  ImGuiPlotArrayGetterData data(values, stride);
  Plot(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void *)&data,
       values_count, values_offset, overlay_text, scale_min, scale_max,
       graph_size);
}
...
void ImGui::PlotHistogram(const char *label,
                          ...
                          const char *overlay_text, float scale_min,
                          float scale_max, ImVec2 graph_size) {
  Plot(ImGuiPlotType_Histogram, label, values_getter, data, values_count,
       values_offset, overlay_text, scale_min, scale_max, graph_size);
}
...
bool ImGui::Checkbox(const char *label, bool *v) {
  ...
  const ImGuiAabb check_bb(window->DC.CursorPos,
                           window->DC.CursorPos +
                               ImVec2(text_size.y + style.FramePadding.y * 2,
                                      text_size.y + style.FramePadding.y * 2));
  ItemSize(check_bb);
  ...
  if (text_size.x > 0) {
    ...
    total_bb = ImGuiAabb(ImMin(check_bb.Min, text_bb.Min),
  ...
  RenderFrame(check_bb.Min, check_bb.Max,
              window->Color((held && hovered) ? ImGuiCol_CheckActive
                                              : hovered ? ImGuiCol_CheckHovered
                                                        : ImGuiCol_FrameBg),
...
bool ImGui::RadioButton(const char *label, bool active) {
  ...
  window->DrawList->AddCircleFilled(
      ...
      window->Color((held && hovered)
                        ...
                        : hovered ? ImGuiCol_CheckHovered : ImGuiCol_FrameBg),
      16);
  if (active) {
    const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
    const float pad = check_sz < 8.0f ? 1.0f : check_sz < 13.0f ? 2.0f : 3.0f;
    window->DrawList->AddCircleFilled(center, radius - pad,
                                      window->Color(ImGuiCol_CheckMark), 16);
  }

  if (window->Flags & ImGuiWindowFlags_ShowBorders) {
    window->DrawList->AddCircle(center + ImVec2(1, 1), radius,
                                window->Color(ImGuiCol_BorderShadow), 16);
    window->DrawList->AddCircle(center, radius, window->Color(ImGuiCol_Border),
                                16);
  }
...
static float STB_TEXTEDIT_GETWIDTH(STB_TEXTEDIT_STRING *obj, int line_start_idx,
                                   int char_idx) {
  (void)line_start_idx;
  return obj->Font->CalcTextSizeW(obj->FontSize, FLT_MAX, &obj->Text[char_idx],
                                  &obj->Text[char_idx] + 1, NULL).x;
}
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, STB_TEXTEDIT_STRING *obj,
  ...
  const ImVec2 size = obj->Font->CalcTextSizeW(obj->FontSize, FLT_MAX,
...
static bool is_white(unsigned int c) {
...
#define STB_TEXTEDIT_IS_SPACE(CH)                                              \
  (is_white((unsigned int)CH) || is_separator((unsigned int)CH))
...
static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING *obj, int pos,
  ...
  if (pos != (int)text_len)
    memmove(obj->Text + (size_t)pos + new_text_len, obj->Text + (size_t)pos,
            (text_len - (size_t)pos) * sizeof(ImWchar));
  memcpy(obj->Text + (size_t)pos, new_text,
         (size_t)new_text_len * sizeof(ImWchar));
...
void ImGuiTextEditState::UpdateScrollOffset() {
  ...
  const float cursor_offset_x =
      Font->CalcTextSizeW(FontSize, FLT_MAX, Text, Text + StbState.cursor, NULL)
          .x;
...
ImVec2 ImGuiTextEditState::CalcDisplayOffsetFromCharIdx(int i) const {
  const ImWchar *text_start =
      GetTextPointerClippedW(Font, FontSize, Text, ScrollX, NULL);
  ...
  const ImVec2 offset =
      Font->CalcTextSizeW(FontSize, Width, text_start, text_end, NULL);
  return offset;
}

// [Static]
const char *ImGuiTextEditState::GetTextPointerClippedA(ImFont *font,
                                                       float font_size,
                                                       const char *text,
                                                       float width,
  ...
  const ImVec2 text_size = font->CalcTextSizeA(font_size, width, 0.0f, text,
                                               NULL, &text_clipped_end);
  if (out_text_size)
...
// [Static]
const ImWchar *
ImGuiTextEditState::GetTextPointerClippedW(ImFont *font, float font_size,
                                           const ImWchar *text, float width,
                                           ImVec2 *out_text_size) {
  if (width <= 0.0f)
    return text;
  ...
  const ImVec2 text_size =
      font->CalcTextSizeW(font_size, width, text, NULL, &text_clipped_end);
  if (out_text_size)
    *out_text_size = text_size;
  return text_clipped_end;
}
...
// [Static]
void ImGuiTextEditState::RenderTextScrolledClipped(ImFont *font,
                                                   float font_size,
                                                   const char *buf, ImVec2 pos,
                                                   float width,
                                                   float scroll_x) {
  // NB- We start drawing at character boundary
  ImVec2 text_size;
  const char *text_start =
      GetTextPointerClippedA(font, font_size, buf, scroll_x, NULL);
  const char *text_end =
      GetTextPointerClippedA(font, font_size, text_start, width, &text_size);

  // Draw a little clip symbol if we've got text on either left or right of the
  // box
  const char symbol_c = '~';
  const float symbol_w = font_size * 0.40f; // FIXME: compute correct width
  const float clip_begin =
      (text_start > buf && text_start < text_end) ? symbol_w : 0.0f;
  const float clip_end =
      (text_end[0] != '\0' && text_end > text_start) ? symbol_w : 0.0f;

  // Draw text
  RenderText(pos + ImVec2(clip_begin, 0),
             text_start + (clip_begin > 0.0f ? 1 : 0),
             text_end - (clip_end > 0.0f ? 1 : 0), false);
...
bool ImGui::InputFloat(const char *label, float *v, float step, float step_fast,
  ...
  if (step > 0.0f)
    ImGui::PushItemWidth(
        ImMax(1.0f, w -
                        (button_sz + style.FramePadding.x * 2.0f +
                         style.ItemInnerSpacing.x) *
                            2));

  char buf[64];
  if (decimal_precision < 0)
    ImFormatString(buf, IM_ARRAYSIZE(buf), "%f",
                   *v); // Ideally we'd have a minimum decimal precision of 1 to
                        // visually denote that it is a float, while hiding
...
bool ImGui::InputInt(const char *label, int *v, int step, int step_fast,
  ...
  const bool value_changed = ImGui::InputFloat(
      label, &f, (float)step, (float)step_fast, 0, extra_flags);
  if (value_changed)
    *v = (int)f;
  return value_changed;
}
...
bool ImGui::InputText(const char *label, char *buf, size_t buf_size,
  ...
  const bool focus_requested = window->FocusItemRegister(
      g.ActiveId == id,
  ...
  if (focus_requested || user_clicked) {
    if (g.ActiveId != id) {
      ...
      ImFormatString(edit_state.InitialText,
                     IM_ARRAYSIZE(edit_state.InitialText), "%s", buf);
      size_t buf_len = ImTextStrFromUtf8(
          edit_state.Text, IM_ARRAYSIZE(edit_state.Text), buf, NULL);
      edit_state.Width = w;
      ...
      if (edit_state.Id != id) {
        ...
        edit_state.StbState.select_start =
            ImMin(edit_state.StbState.select_start, buf_len);
        edit_state.StbState.select_end =
            ImMin(edit_state.StbState.select_end, buf_len);
      }
  ...
  // Down the line we should have a cleaner concept of focused vs active in the
  // library.
  if (g.ActiveId == id)
  ...
  if (g.ActiveId == id) {
    // Edit in progress
    edit_state.BufSize = buf_size < IM_ARRAYSIZE(edit_state.Text)
                             ? buf_size
                             : IM_ARRAYSIZE(edit_state.Text);
    ...
    const float my =
        window->FontSize() *
        0.5f; // Flatten mouse because we are doing a single-line edit
    ...
    if (select_all || (hovered && io.MouseDoubleClicked[0])) {
      ...
      stb_textedit_click(&edit_state, &edit_state.StbState,
                         mx + edit_state.ScrollX, my);
      edit_state.CursorAnimReset();

    } else if (io.MouseDown[0] && !edit_state.SelectedAllMouseLock) {
      stb_textedit_drag(&edit_state, &edit_state.StbState,
                        mx + edit_state.ScrollX, my);
      edit_state.CursorAnimReset();
    }
    ...
    if (IsKeyPressedMap(ImGuiKey_LeftArrow)) {
      ...
      edit_state.SelectAll();
               ...
               (IsKeyPressedMap(ImGuiKey_X) || IsKeyPressedMap(ImGuiKey_C))) {
      // Cut, Copy
      ...
      if (g.IO.SetClipboardTextFn) {
        ...
        const int ie = edit_state.HasSelection()
                           ? ImMax(edit_state.StbState.select_start,
                                   edit_state.StbState.select_end)
                           : (int)ImStrlenW(edit_state.Text);
        ImTextStrToUtf8(text_tmp_utf8, IM_ARRAYSIZE(text_tmp_utf8),
                        edit_state.Text + ib, edit_state.Text + ie);
    ...
    if (cancel_edit) {
      ...
      // Note that as soon as we can focus into the input box, the in-widget
      // value gets priority over any underlying modification of the input
      // buffer
      ...
      if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
        ...
        if (event_key != ImGuiKey_COUNT ||
          ...
          const int utf8_cursor_pos = callback_data.CursorPos =
              ImTextCountUtf8BytesFromWchar(edit_state.Text,
                                            edit_state.Text +
                                                edit_state.StbState.cursor);
          const int utf8_selection_start = callback_data.SelectionStart =
              ImTextCountUtf8BytesFromWchar(
                  edit_state.Text,
                  edit_state.Text + edit_state.StbState.select_start);
          ...
          IM_ASSERT(callback_data.Buf ==
                    text_tmp_utf8); // Invalid to modify those fields
          ...
          if (callback_data.BufDirty) {
            ImTextStrFromUtf8(edit_state.Text, IM_ARRAYSIZE(edit_state.Text),
                              text_tmp_utf8, NULL);
            edit_state.CursorAnimReset();
          }
        }
  ...
  const ImVec2 font_off_up = ImVec2(
      0.0f, window->FontSize() +
                1.0f); // FIXME: those offsets are part of the style or font API
  ...
  if (g.ActiveId == id) {
    ...
    if (select_begin_idx != select_end_idx) {
      const ImVec2 select_begin_pos =
          frame_bb.Min + style.FramePadding +
          edit_state.CalcDisplayOffsetFromCharIdx(
              ImMin(select_begin_idx, select_end_idx));
      const ImVec2 select_end_pos =
          frame_bb.Min + style.FramePadding +
          edit_state.CalcDisplayOffsetFromCharIdx(
              ImMax(select_begin_idx, select_end_idx));
      window->DrawList->AddRectFilled(select_begin_pos - font_off_up,
                                      select_end_pos + font_off_dn,
                                      window->Color(ImGuiCol_TextSelectedBg));
    }
  }

  ImGuiTextEditState::RenderTextScrolledClipped(
      window->Font(), window->FontSize(), buf,
  ...
  if (g.ActiveId == id) {
    ...
    if (g.InputTextState.CursorIsVisible())
      window->DrawList->AddRect(cursor_pos - font_off_up + ImVec2(0, 2),
                                cursor_pos + font_off_dn - ImVec2(0, 3),
                                window->Color(ImGuiCol_Text));

    // Notify OS of text input position for advanced IME
    if (io.ImeSetInputScreenPosFn &&
        ImLengthSqr(edit_state.InputCursorScreenPos - cursor_pos) > 0.0001f)
...
static bool InputFloatN(const char *label, float *v, int components,
...
bool ImGui::Combo(const char *label, int *current_item,
  ...
  const ImGuiAabb bb(frame_bb.Min,
                     frame_bb.Max +
                         ImVec2(style.ItemInnerSpacing.x + text_size.x, 0));
  const float arrow_size = (window->FontSize() + style.FramePadding.x * 2.0f);
  const bool hovered = IsHovered(frame_bb, id);

  bool value_changed = false;
  const ImGuiAabb value_bb(frame_bb.Min,
  ...
  if (*current_item >= 0 && *current_item < items_count) {
    ...
    if (items_getter(data, *current_item, &item_text))
      RenderTextClipped(frame_bb.Min + style.FramePadding, item_text, NULL,
                        NULL, value_bb.Max);
  }
  ...
  if (g.ActiveComboID == id) {
    ...
    const float popup_height = (text_size.y + style.ItemSpacing.y) *
                                   ImMin(items_count, height_in_items) +
                               style.WindowPadding.y;
    const ImGuiAabb popup_aabb(
...
bool ImGui::Selectable(const char *label, bool selected,
  ...
  const float w =
      window->Pos.x + ImGui::GetContentRegionMax().x - window->DC.CursorPos.x;
  const ImVec2 size(size_arg.x != 0.0f ? size_arg.x : w,
                    size_arg.y != 0.0f ? size_arg.y : text_size.y);
  const ImGuiAabb bb(window->DC.CursorPos, window->DC.CursorPos + size);
  ItemSize(bb);

  // Selectables are meant to be tightly packed together. So for both rendering
  // and collision we extend to compensate for spacing.
  ...
  bool hovered, held;
  bool pressed =
      ButtonBehaviour(bb_with_spacing, id, &hovered, &held, true, false, false);
  ...
  // const ImVec2 off = ImVec2(ImMax(0.0f, size.x - text_size.x) * 0.5f,
  // ImMax(0.0f, size.y - text_size.y) * 0.5f);
...
bool ImGui::ListBoxHeader(const char *label, const ImVec2 &size_arg) {
  ...
  size.y = (size_arg.y != 0.0f) ? size_arg.y
                                : ImGui::GetTextLineHeightWithSpacing() * 7.4f +
                                      style.ItemSpacing.y;
...
bool ImGui::ListBoxHeader(const char *label, int items_count,
  ...
  ImVec2 size;
  size.x = 0.0f;
  size.y = ImGui::GetTextLineHeightWithSpacing() * height_in_items_f +
           ImGui::GetStyle().ItemSpacing.y;
  return ImGui::ListBoxHeader(label, size);
}

void ImGui::ListBoxFooter() { ImGui::EndChildFrame(); }
...
bool ImGui::ColorButton(const ImVec4 &col, bool small_height,
  ...
  const ImGuiAabb bb(
      window->DC.CursorPos,
      window->DC.CursorPos +
          ImVec2(square_size + style.FramePadding.x * 2,
...
bool ImGui::ColorEdit4(const char *label, float col[4], bool alpha) {
  ...
  case ImGuiColorEditMode_HSV: {
    ...
    ImGui::PushItemWidth(w_item_one);
    value_changed |=
        ImGui::SliderInt("##X", &ix, 0, 255, hsv ? "H:%3.0f" : "R:%3.0f");
    ImGui::SameLine(0, (int)style.ItemInnerSpacing.x);
    value_changed |=
        ImGui::SliderInt("##Y", &iy, 0, 255, hsv ? "S:%3.0f" : "G:%3.0f");
    ImGui::SameLine(0, (int)style.ItemInnerSpacing.x);
    if (alpha) {
      value_changed |=
          ImGui::SliderInt("##Z", &iz, 0, 255, hsv ? "V:%3.0f" : "B:%3.0f");
      ImGui::SameLine(0, (int)style.ItemInnerSpacing.x);
      ImGui::PushItemWidth(w_item_last);
  ...
  case ImGuiColorEditMode_HEX: {
    ...
    ix = iy = iz = iw = 0;
    if (alpha)
      sscanf(p, "%02X%02X%02X%02X", (unsigned int *)&ix, (unsigned int *)&iy,
             (unsigned int *)&iz, (unsigned int *)&iw);
    else
      sscanf(p, "%02X%02X%02X", (unsigned int *)&ix, (unsigned int *)&iy,
             (unsigned int *)&iz);
  } break;
  }
...
void ImGui::Separator() {
  ...
  ItemSize(ImVec2(0.0f, bb.GetSize().y)); // NB: we don't provide our width so
                                          // that it doesn't get feed back into
                                          // AutoFit
  if (!ItemAdd(bb, NULL)) {
    if (window->DC.ColumnsCount > 1)
      PushColumnClipRect();
...
static void ItemSize(ImVec2 size, ImVec2 *adjust_vertical_offset) {
  ...
  if (adjust_vertical_offset)
    adjust_vertical_offset->y =
        adjust_vertical_offset->y + (line_height - size.y) * 0.5f;
  ...
  window->SizeContentsFit =
      ImMax(window->SizeContentsFit,
            ImVec2(window->DC.CursorPosPrevLine.x, window->DC.CursorPos.y) -
                window->Pos + ImVec2(0.0f, window->ScrollY));
...
bool ImGui::IsClipped(const ImVec2 &item_size) {
  ImGuiWindow *window = GetCurrentWindow();
  return IsClipped(
      ImGuiAabb(window->DC.CursorPos, window->DC.CursorPos + item_size));
}
...
static bool ItemAdd(const ImGuiAabb &bb, const ImGuiID *id) {
  ...
  // const bool hovered = (g.ActiveId == 0 || (id && g.ActiveId == *id) ||
  // g.ActiveIdIsFocusedOnly) && IsMouseHoveringBox(bb);  // matching the
  // behaviour of IsHovered(), not always what the user wants?
...
//      spacing_w < 0   : use default spacing if column_x==0, no spacing if
//      column_x!=0
...
void ImGui::SameLine(int column_x, int spacing_w) {
  ...
  if (column_x != 0) {
    ...
    if (spacing_w < 0)
      spacing_w = (int)g.Style.ItemSpacing.x;
...
void ImGui::NextColumn() {
  ...
  if (window->DC.ColumnsCount > 1) {
    ...
    if (++window->DC.ColumnsCurrent < window->DC.ColumnsCount) {
      window->DC.ColumnsOffsetX =
          ImGui::GetColumnOffset(window->DC.ColumnsCurrent) -
    ...
    window->DC.CursorPos.x = (float)(int)(
        window->Pos.x + window->DC.ColumnsStartX + window->DC.ColumnsOffsetX);
...
float ImGui::GetColumnOffset(int column_index) {
  ...
  const float max_x =
      window->Size.x - (g.Style.ScrollBarWidth); // - window->WindowPadding().x;
...
float ImGui::GetColumnWidth(int column_index) {
  ...
  const float w =
      GetColumnOffset(column_index + 1) - GetColumnOffset(column_index);
  return w;
}
...
void ImGui::Columns(int columns_count, const char *id, bool border) {
  ...
  if (window->DC.ColumnsCount != columns_count &&
    ...
    for (int i = 1; i < window->DC.ColumnsCount; i++) {
      ...
      // being drawn
      const ImU32 col = window->Color(held ? ImGuiCol_ColumnActive
  ...
  if (window->DC.ColumnsCount != 1) {
    ...
    for (int column_index = 0; column_index < columns_count + 1;
      ...
      const float t = window->StateStorage.GetFloat(
...
void ImDrawList::AddDrawCmd() {
  ...
  draw_cmd.clip_rect =
      clip_rect_stack.empty() ? GNullClipRect : clip_rect_stack.back();
  draw_cmd.texture_id =
      texture_id_stack.empty() ? NULL : texture_id_stack.back();
...
void ImDrawList::AddCallback(ImDrawCallback callback, void *callback_data) {
  ...
  if (!current_cmd || current_cmd->vtx_count != 0 ||
      current_cmd->user_callback != NULL) {
    AddDrawCmd();
  ...
  // We function this way so that the most common calls (AddLine, AddRect..)
  // always have a command to add to without doing any check.
  AddDrawCmd();
}
...
void ImDrawList::UpdateTextureID() {
  ...
  const ImTextureID texture_id =
      texture_id_stack.empty() ? NULL : texture_id_stack.back();
  if (!current_cmd ||
      (current_cmd->vtx_count != 0 && current_cmd->texture_id != texture_id) ||
      current_cmd->user_callback != NULL) {
    AddDrawCmd();
  } else {
...
void ImDrawList::AddVtxLine(const ImVec2 &a, const ImVec2 &b, ImU32 col) {
  ...
  // Two triangles makes up one line. Using triangles allows us to reduce amount
  // of draw calls.
  AddVtx(a + hp0, col);
  AddVtx(b + hp0, col);
  AddVtx(a + hp1, col);
...
void ImDrawList::AddArc(const ImVec2 &center, float rad, ImU32 col, int a_min,
                        int a_max, bool tris,
  ...
  if (tris) {
    ...
    for (int a = a_min; a < a_max; a++) {
      ...
      AddVtx(center + circle_vtx[(a + 1) % IM_ARRAYSIZE(circle_vtx)] * rad,
             col);
    ...
    for (int a = a_min; a < a_max; a++)
      AddVtxLine(center + circle_vtx[a % IM_ARRAYSIZE(circle_vtx)] * rad,
                 center + circle_vtx[(a + 1) % IM_ARRAYSIZE(circle_vtx)] * rad,
                 col);
  }
}
...
void ImDrawList::AddRect(const ImVec2 &a, const ImVec2 &b, ImU32 col,
  ...
  if (r == 0.0f || rounding_corners == 0) {
    ...
    if (rounding_corners & 1)
      AddArc(ImVec2(a.x + r, a.y + r), r, col, 0, 3);
    if (rounding_corners & 2)
      AddArc(ImVec2(b.x - r, a.y + r), r, col, 3, 6);
    if (rounding_corners & 4)
      AddArc(ImVec2(b.x - r, b.y - r), r, col, 6, 9);
    if (rounding_corners & 8)
      AddArc(ImVec2(a.x + r, b.y - r), r, col, 9, 12);
  }
}
...
void ImDrawList::AddRectFilled(const ImVec2 &a, const ImVec2 &b, ImU32 col,
  ...
  if (r == 0.0f || rounding_corners == 0) {
    // Use triangle so we can merge more draw calls together (at the cost of
    // extra vertices)
    ReserveVertices(6);
    ...
    if (rounding_corners & 1)
      AddArc(ImVec2(a.x + r, a.y + r), r, col, 0, 3, true);
    if (rounding_corners & 2)
      AddArc(ImVec2(b.x - r, a.y + r), r, col, 3, 6, true);
    if (rounding_corners & 4)
      AddArc(ImVec2(b.x - r, b.y - r), r, col, 6, 9, true);
    if (rounding_corners & 8)
      AddArc(ImVec2(a.x + r, b.y - r), r, col, 9, 12, true);
  }
}
...
void ImDrawList::AddCircle(const ImVec2 &centre, float radius, ImU32 col,
  ...
  for (int i = 0; i < num_segments; i++) {
    ...
    AddVtxLine(centre + ImVec2(cosf(a0), sinf(a0)) * radius,
               centre + ImVec2(cosf(a1), sinf(a1)) * radius, col);
    a0 = a1;
  }
}
...
void ImDrawList::AddText(ImFont *font, float font_size, const ImVec2 &pos,
  ...
  IM_ASSERT(font->ContainerAtlas->TexID ==
            texture_id_stack.back()); // Use high-level ImGui::PushFont() or
  ...
  font->RenderText(font_size, pos, col, clip_rect_stack.back(), text_begin,
                   text_end, vtx_write, wrap_width, cpu_clip_max);
...
void ImDrawList::AddImage(ImTextureID user_texture_id, const ImVec2 &a,
  ...
  const bool push_texture_id =
      texture_id_stack.empty() || user_texture_id != texture_id_stack.back();
  if (push_texture_id)
...
struct ImFontAtlas::ImFontAtlasData {
  ...
  const ImWchar *GlyphRanges; // List of Unicode range (2 value per range,
...
void ImGui::GetDefaultFontData(const void **fnt_data, unsigned int *fnt_size,
                               const void **png_data, unsigned int *png_size) {
  printf("GetDefaultFontData() is obsoleted in ImGui 1.30.\n");
  printf("Please use ImGui::GetIO().Fonts->GetTexDataAsRGBA32() or "
         "GetTexDataAsAlpha8() functions to retrieve uncompressed texture "
         "data.\n");
  if (fnt_data)
    *fnt_data = NULL;
  if (fnt_size)
...
ImFont *ImFontAtlas::AddFontDefault() {
  ...
  // Decompress
  const size_t buf_decompressed_size =
      stb_decompress_length((unsigned char *)ttf_compressed);
  unsigned char *buf_decompressed =
      (unsigned char *)ImGui::MemAlloc(buf_decompressed_size);
  stb_decompress(buf_decompressed, (unsigned char *)ttf_compressed,
                 ttf_compressed_size);

  // Add
  ImFont *font = AddFontFromMemoryTTF(buf_decompressed, buf_decompressed_size,
                                      13.0f, GetGlyphRangesDefault(), 0);
...
ImFont *ImFontAtlas::AddFontFromFileTTF(const char *filename, float size_pixels,
  ...
  // Add
  ImFont *font =
      AddFontFromMemoryTTF(data, data_size, size_pixels, glyph_ranges, font_no);
  return font;
}
...
ImFont *ImFontAtlas::AddFontFromMemoryTTF(void *in_ttf_data,
  ...
  // Add to build list
  ImFontAtlasData *data =
      (ImFontAtlasData *)ImGui::MemAlloc(sizeof(ImFontAtlasData));
...
bool ImFontAtlas::Build() {
  ...
  for (size_t input_i = 0; input_i < InputData.size(); input_i++) {
    ...
    const int font_offset =
        stbtt_GetFontOffsetForIndex((unsigned char *)data.TTFData, data.FontNo);
    IM_ASSERT(font_offset >= 0);
    if (!stbtt_InitFont(&data.FontInfo, (unsigned char *)data.TTFData,
                        font_offset))
      return false;

    if (!data.GlyphRanges)
      data.GlyphRanges = GetGlyphRangesDefault();
    for (const ImWchar *in_range = data.GlyphRanges; in_range[0] && in_range[1];
         in_range += 2) {
  ...
  // Start packing
  TexWidth = (total_glyph_count > 1000)
                 ? 1024
                 : 512; // Width doesn't actually matters.
  TexHeight = 0;
  ...
  // Pack our extra data rectangle first, so it will be on the upper-left corner
  ...
  memset(buf_rects, 0, total_glyph_count * sizeof(stbrp_rect)); // Unnessary but
                                                                // let's clear
                                                                // this for the
                                                                // sake of
                                                                // sanity.
  ...
  // First pass: pack all glyphs (no rendering at this point, we are working
  // with glyph sizes only)
  ...
  for (size_t input_i = 0; input_i < InputData.size(); input_i++) {
    ...
    const int n = stbtt_PackFontRangesGatherRects(
        &spc, &data.FontInfo, data.Ranges, data.RangesCount, data.Rects);
  ...
  for (size_t input_i = 0; input_i < InputData.size(); input_i++) {
    ImFontAtlasData &data = *InputData[input_i];
    ret = stbtt_PackFontRangesRenderIntoRects(&spc, &data.FontInfo, data.Ranges,
                                              data.RangesCount, data.Rects);
    data.Rects = NULL;
  }
  ...
  for (size_t input_i = 0; input_i < InputData.size(); input_i++) {
    ...
    const float font_scale =
        stbtt_ScaleForPixelHeight(&data.FontInfo, data.SizePixels);
    int font_ascent, font_descent, font_line_gap;
    stbtt_GetFontVMetrics(&data.FontInfo, &font_ascent, &font_descent,
                          &font_line_gap);
    ...
    for (int i = 0; i < data.RangesCount; i++) {
      stbtt_pack_range &range = data.Ranges[i];
      for (int char_idx = 0; char_idx < range.num_chars_in_range;
           char_idx += 1) {
        ...
        glyph.YOffset =
            (signed short)(pc.yoff + (int)(font_ascent * font_scale));
        glyph.XAdvance =
            (signed short)(pc.xadvance +
                           character_spacing_x); // Bake spacing into XAdvance
...
void ImFontAtlas::RenderCustomTexData() {
  ...
  TexPixelsAlpha8[0] = TexPixelsAlpha8[1] = TexPixelsAlpha8[TexWidth + 0] =
      TexPixelsAlpha8[TexWidth + 1] = 0xFF;
  TexUvWhitePixel = ImVec2((TexExtraDataPos.x + 0.5f) / TexWidth,
                           (TexExtraDataPos.y + 0.5f) / TexHeight);

  // Draw a mouse cursor into texture
  // Because our font uses an alpha texture, we have to spread the cursor in 2
  // parts (black/white) which will be rendered separately.
  const char cursor_pixels[] = {"X           "
                                "XX          "
                                "X.X         "
                                "X..X        "
                                "X...X       "
                                ...
                                "      X..X  "
                                "       XX   "};
  IM_ASSERT(sizeof(cursor_pixels) - 1 ==
            (int)TEX_ATLAS_SIZE_MOUSE_CURSOR.x *
                (int)TEX_ATLAS_SIZE_MOUSE_CURSOR.y);
  for (int y = 0, n = 0; y < 19; y++)
    for (int x = 0; x < 12; x++, n++) {
      TexPixelsAlpha8[((int)TEX_ATLAS_POS_MOUSE_CURSOR_BLACK.x + x) +
                      ((int)TEX_ATLAS_POS_MOUSE_CURSOR_BLACK.y + y) *
                          TexWidth] = (cursor_pixels[n] == 'X') ? 0xFF : 0x00;
      TexPixelsAlpha8[((int)TEX_ATLAS_POS_MOUSE_CURSOR_WHITE.x + x) +
                      ((int)TEX_ATLAS_POS_MOUSE_CURSOR_WHITE.y + y) *
                          TexWidth] = (cursor_pixels[n] == '.') ? 0xFF : 0x00;
    }
}
...
void ImFont::BuildLookupTable() {
  ...
  // FIXME: Needs proper TAB handling but it needs to be contextualized (can
...
static int ImTextCharFromUtf8(unsigned int *out_char, const char *in_text,
  ...
  if ((*str & 0xf0) == 0xe0) {
    ...
    if (*str == 0xed && str[1] > 0x9f)
      return -1; // str[1] < 0x80 is checked below
...
static ptrdiff_t ImTextStrFromUtf8(ImWchar *buf, size_t buf_size,
                                   const char *in_text,
                                   const char *in_text_end) {
  ImWchar *buf_out = buf;
...
static ptrdiff_t ImTextStrToUtf8(char *buf, size_t buf_size,
  ...
  while (buf_out < buf_end - 1 && (!in_text_end || in_text < in_text_end) &&
         *in_text) {
    buf_out += ImTextCharToUtf8(buf_out, (uintptr_t)(buf_end - buf_out - 1),
...
static int ImTextCountUtf8BytesFromWchar(const ImWchar *in_text,
                                         const ImWchar *in_text_end) {
  int bytes_count = 0;
  while ((!in_text_end || in_text < in_text_end) && *in_text) {
...
const char *ImFont::CalcWordWrapPositionA(float scale, const char *text,
  ...
  while (s < text_end) {
    ...
    const float char_width = ((size_t)c < IndexXAdvance.size())
                                 ? IndexXAdvance[(size_t)c] * scale
                                 : 0.0f;
    if (c == ' ' || c == '\t') {
      if (inside_word) {
...
ImVec2 ImFont::CalcTextSizeA(float size, float max_width, float wrap_width,
  ...
  while (s < text_end) {
    ...
    // Decode and advance source (handle unlikely UTF-8 decoding failure by
    // skipping to the next byte)
    unsigned int c = (unsigned int)*s;
    if (c < 0x80)
      s += 1;
    else
...
ImVec2 ImFont::CalcTextSizeW(float size, float max_width,
                             const ImWchar *text_begin, const ImWchar *text_end,
                             const ImWchar **remaining) const {
  if (!text_end)
...
void ImFont::RenderText(float size, ImVec2 pos, ImU32 col,
                        const ImVec4 &clip_rect_ref, const char *text_begin,
                        const char *text_end, ImDrawVert *&out_vertices,
                        float wrap_width, const ImVec2 *cpu_clip_max) const {
  if (!text_end)
    text_end = text_begin + strlen(text_begin);
      ...
      if (c != ' ' && c != '\t') {
        ...
        if (y1 <= clip_rect.w && y2 >= clip_rect.y) {
          ...
          if (x1 <= clip_rect.z && x2 >= clip_rect.x) {
            ...
            // CPU side clipping used to fit text in their frame when the frame
            // is too small. Only does clipping for axis aligned quad and in the
            // "max" direction (bottom-right)
            if (cpu_clip_max) {
...
static void SetClipboardTextFn_DefaultImpl(const char *text) {
  ...
  HGLOBAL buf_handle =
      GlobalAlloc(GMEM_MOVEABLE, (SIZE_T)buf_length * sizeof(char));
  if (buf_handle == NULL)
    return;
...
void ImGui::ShowStyleEditor(ImGuiStyle *ref) {
  ...
  if (ImGui::TreeNode("Sizes")) {
    ImGui::SliderFloat("Alpha", &style.Alpha, 0.20f, 1.0f,
                       "%.2f"); // Not exposing zero here so user doesn't "lose"
                                // the UI. But application code could have a
    ...
    ImGui::SliderFloat2("TouchExtraPadding", (float *)&style.TouchExtraPadding,
                        0.0f, 10.0f, "%.0f");
    ImGui::SliderFloat("TreeNodeSpacing", &style.TreeNodeSpacing, 0.0f, 20.0f,
                       "%.0f");
    ImGui::SliderFloat("ScrollBarWidth", &style.ScrollBarWidth, 0.0f, 20.0f,
                       "%.0f");
    ImGui::TreePop();
  }
      ...
      for (int i = 0; i < ImGuiCol_COUNT; i++) {
        ...
        if (!output_only_modified ||
          ...
          ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, "
                         "%.2ff, %.2ff);" STR_NEWLINE,
                         name, 22 - strlen(name), "", col.x, col.y, col.z,
                         col.w);
      }
      ImGui::LogFinish();
    }
...
// Demonstrate ImGui features (unfortunately this makes this function a little
// bloated!)
void ImGui::ShowTestWindow(bool *opened) {
  ...
  if (!ImGui::Begin("ImGui Test", opened, ImVec2(550, 680), bg_alpha,
                    window_flags)) {
  ...
  ImGui::PushItemWidth(ImGui::GetWindowWidth() *
                       0.65f); // 2/3 of the space for widget and 1/3 for labels
  // ImGui::PushItemWidth(-140);                             // Right align,
  // keep 140 pixels for labels

  ImGui::Text("ImGui says hello.");
  // ImGui::Text("MousePos (%g, %g)", ImGui::GetIO().MousePos.x,
  ...
  if (ImGui::CollapsingHeader("Window options")) {
    ...
    if (ImGui::TreeNode("Fonts")) {
      ImGui::TextWrapped(
          "Tip: Load fonts with GetIO().Fonts->AddFontFromFileTTF().");
      for (size_t i = 0; i < ImGui::GetIO().Fonts->Fonts.size(); i++) {
        ImFont *font = ImGui::GetIO().Fonts->Fonts[i];
        ImGui::BulletText("Font %d: %.2f pixels, %d glyphs", i, font->FontSize,
                          font->Glyphs.size());
        ...
        ImGui::SliderFloat("font scale", &font->Scale, 0.3f, 2.0f,
                           "%.1f"); // scale only this font
        ImGui::TreePop();
      }
      static float window_scale = 1.0f;
      ImGui::SliderFloat("this window scale", &window_scale, 0.3f, 2.0f,
                         "%.1f"); // scale only this window
      ImGui::SliderFloat("global scale", &ImGui::GetIO().FontGlobalScale, 0.3f,
                         2.0f, "%.1f"); // scale everything
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    ...
    if (ImGui::TreeNode("Word Wrapping")) {
      ...
      ImGui::TextWrapped("This text should automatically wrap on the edge of "
                         ...
                         "wrapping follows simple rules that works for English "
      ...
      ImGui::GetWindowDrawList()->AddRectFilled(
          ImGui::GetCursorScreenPos() + ImVec2(wrap_width, 0.0f),
          ImGui::GetCursorScreenPos() +
              ImVec2(wrap_width + 10, ImGui::GetTextLineHeight()),
          0xFFFF00FF);
      ImGui::PushTextWrapPos(ImGui::GetCursorPos().x + wrap_width);
      ImGui::Text("lazy dog. This paragraph is made to fit within %.0f pixels. "
                  "The quick brown fox jumps over the lazy dog.",
                  wrap_width);
      ImGui::GetWindowDrawList()->AddRect(ImGui::GetItemBoxMin(),
                                          ImGui::GetItemBoxMax(), 0xFF00FFFF);
      ImGui::PopTextWrapPos();
    ...
    if (ImGui::TreeNode("Clipping")) {
      ...
      ImGui::TextWrapped("On a per-widget basis we are occasionally clipping "
                         "text if it won't fit in its frame.");
      ImGui::SliderFloat2("size", (float *)&size, 5.0f, 200.0f);
      ImGui::Button("Line 1 hello\nLine 2 clip me!", size);
      ImGui::TextWrapped("Otherwise we are doing coarser clipping + passing a "
    ...
    if (ImGui::TreeNode("Images")) {
      ...
      if (ImGui::IsItemHovered()) {
        ...
        float focus_x =
            ImClamp(ImGui::GetMousePos().x - tex_screen_pos.x - focus_sz * 0.5f,
                    0.0f, tex_w - focus_sz);
        float focus_y =
            ImClamp(ImGui::GetMousePos().y - tex_screen_pos.y - focus_sz * 0.5f,
                    0.0f, tex_h - focus_sz);
  ...
  if (ImGui::CollapsingHeader("Graphs widgets")) {
    ...
    ImGui::PlotLines("Frame Times", &values.front(), (int)values.size(),
                     (int)values_offset, "avg 0.0", -1.0f, 1.0f, ImVec2(0, 70));
  ...
  if (ImGui::CollapsingHeader("Columns")) {
    ...
    const char *paths[3] = {"/path/robert", "/path/stephanie",
                            "/path/computer"};
    for (int i = 0; i < 3; i++) {
      ImGui::Text("%04d", i);
      ImGui::NextColumn();
...
ImGui::Spacing();
  ...
  if (ImGui::CollapsingHeader("Keyboard & Focus")) {
    if (ImGui::TreeNode("Tabbing")) {
      ...
      // ImGui::SameLine(); ImGui::Text("(?)"); if (ImGui::IsHovered())
      // ImGui::SetTooltip("Use ImGui::PushAllowKeyboardFocus(bool)\nto disable
...
static void ShowExampleAppAutoResize(bool *opened) {
  if (!ImGui::Begin("Example: Auto-Resizing Window", opened, ImVec2(0, 0),
                    -1.0f, ImGuiWindowFlags_AlwaysAutoResize)) {
    ImGui::End();
    return;
  }

  static int lines = 10;
  ImGui::TextWrapped("Window will resize every-frame to the size of its "
                     "content. Note that you don't want to query the window "
                     "size to output your content because that would create a "
                     "feedback loop.");
...
static void ShowExampleAppCustomRendering(bool *opened) {
  ...
  // However you can draw directly and poll mouse/keyboard by yourself. You can
  // manipulate the cursor using GetCursorPos() and SetCursorPos().
  // If you only use the ImDrawList API, you can notify the owner window of its
  // extends by using SetCursorPos(max).
  ImVec2 canvas_pos =
      ImGui::GetCursorScreenPos(); // ImDrawList API uses screen coordinates!
  ImVec2 canvas_size = ImVec2(
      ImMax(50.0f,
            ImGui::GetWindowContentRegionMax().x - ImGui::GetCursorPos().x),
      ImMax(50.0f,
            ImGui::GetWindowContentRegionMax().y -
                ImGui::GetCursorPos().y)); // Resize canvas what's available
  ...
  draw_list->PushClipRect(
      ImVec4(canvas_pos.x, canvas_pos.y, canvas_pos.x + canvas_size.x,
             canvas_pos.y + canvas_size.y)); // clip lines within the canvas (if
                                             // we resize it, etc.)
...
struct ExampleAppConsole {
  ...
  void Run(const char *title, bool *opened) {
    ...
    if (ImGui::IsItemHovered())
      ImGui::SetKeyboardFocusHere(-1); // Auto focus on hover
    ImGui::PopStyleVar();
    ImGui::Separator();
    ...
    for (size_t i = 0; i < Items.size(); i++) {
      ...
      if (!filter.PassFilter(item))
        continue;
      ImVec4 col(1, 1, 1, 1); // A better implement may store a type per-item.
  ...
  int TextEditCallback(ImGuiTextEditCallbackData *data) {
    ...
    case ImGuiInputTextFlags_CallbackCompletion: {
      ...
      for (size_t i = 0; i < Commands.size(); i++)
        if (ImStrnicmp(Commands[i], word_start, (int)(word_end - word_start)) ==
            0)
...
static void ShowExampleAppLongText(bool *opened) {
  ...
  case 1:
    ...
    // use the CalcListClipping() helper.
    ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0));
    int display_start, display_end;
    ImGui::CalcListClipping(lines, ImGui::GetTextLineHeight(), &display_start,
                            &display_end);
    ...
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() +
                         (lines - display_end) * ImGui::GetTextLineHeight());
    ImGui::PopStyleVar();
    break;
  case 2:
...
static const unsigned int proggy_clean_ttf_compressed_data[9584 / 4] = {
    0x0000bc57, 0x00000000, 0xf8a00000, 0x00000400, 0x00010037, 0x000c0000,
    0x00030080, 0x2f534f40, 0x74eb8832, 0x01000090, 0x2c158248, 0x616d634e,
    0x23120270, 0x03000075, 0x241382a0, 0x74766352, 0x82178220, 0xfc042102,
    0x02380482, 0x66796c67, 0x5689af12, 0x04070000, 0x80920000, 0x64616568,
    0xd36691d7, 0xcc201b82, 0x36210382, 0x27108268, 0xc3014208, 0x04010000,
    0x243b0f82, 0x78746d68, 0x807e008a, 0x98010000, 0x06020000, 0x61636f6c,
    0xd8b0738c, 0x82050000, 0x0402291e, 0x7078616d, 0xda00ae01, 0x28201f82,
    0x78080982, 0x01020101, 0x01040103, 0x01060105, 0x01080107, 0x010a0109,
    0x010c010b, 0x010e010d, 0x0110010f, 0x01120111, 0x01140113, 0x01160115,
    0x01180117, 0x011a0119, 0x011c011b, 0x011e011d, 0x0020011f, 0x00040003,
    0x00060005, 0x00080007, 0x000a0009, 0x000c000b, 0x000e000d, 0x0010000f,
    0x00120011, 0x00140013, 0x00160015, 0x00180017, 0x001a0019, 0x001c001b,
    0x001e001d, 0x08bb821f, 0x22002142, 0x24002300, 0x26002500, 0x28002700,
    0x2a002900, 0x2c002b00, 0x2e002d00, 0x30002f00, 0x32003100, 0x34003300,
    0x36003500, 0x38003700, 0x3a003900, 0x3c003b00, 0x3e003d00, 0x40003f00,
    0x42004100, 0x4b09f382, 0x00450044, 0x00470046, 0x00490048, 0x004b004a,
    0x004d004c, 0x004f004e, 0x00510050, 0x00530052, 0x00550054, 0x00570056,
    0x00590058, 0x005b005a, 0x005d005c, 0x005f005e, 0x01610060, 0x01220121,
    0x01240123, 0x01260125, 0x01280127, 0x012a0129, 0x012c012b, 0x012e012d,
    0x0130012f, 0x01320131, 0x01340133, 0x01360135, 0x01380137, 0x013a0139,
    0x013c013b, 0x013e013d, 0x0140013f, 0x00ac0041, 0x008400a3, 0x00bd0085,
    0x00e80096, 0x008e0086, 0x009d008b, 0x00a400a9, 0x008a00ef, 0x008300da,
    0x00f20093, 0x008d00f3, 0x00880097, 0x00de00c3, 0x009e00f1, 0x00f500aa,
    0x00f600f4, 0x00ad00a2, 0x00c700c9, 0x006200ae, 0x00900063, 0x00cb0064,
    0x00c80065, 0x00cf00ca, 0x00cd00cc, 0x00e900ce, 0x00d30066, 0x00d100d0,
    0x006700af, 0x009100f0, 0x00d400d6, 0x006800d5, 0x00ed00eb, 0x006a0089,
    0x006b0069, 0x006c006d, 0x00a0006e, 0x0071006f, 0x00720070, 0x00750073,
    0x00760074, 0x00ea0077, 0x007a0078, 0x007b0079, 0x007c007d, 0x00a100b8,
    0x007e007f, 0x00810080, 0x00ee00ec, 0x6e750eba, 0x646f6369, 0x78302365,
    0x31303030, 0x32200e8d, 0x33200e8d, 0x34200e8d, 0x35200e8d, 0x36200e8d,
    0x37200e8d, 0x38200e8d, 0x39200e8d, 0x61200e8d, 0x62200e8d, 0x63200e8d,
    0x64200e8d, 0x65200e8d, 0x66200e8d, 0x31210e8c, 0x8d0e8d30, 0x8d3120ef,
    0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef,
    0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef, 0x8d3120ef,
    0x66312def, 0x6c656406, 0x04657465, 0x6f727545, 0x3820ec8c, 0x3820ec8d,
    0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d,
    0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d, 0x3820ec8d,
    0x3820ec8d, 0x200ddc41, 0x0ddc4139, 0xef8d3920, 0xef8d3920, 0xef8d3920,
    0xef8d3920, 0xef8d3920, 0xef8d3920, 0xef8d3920, 0xef8d3920, 0xef8d3920,
    0xef8d3920, 0xef8d3920, 0xef8d3920, 0xef8d3920, 0xef8d3920, 0x00663923,
    0x48fa0500, 0x00f762f9,
};
...
//---- So you can include code that extends ImGui using its private
// data/functions.
