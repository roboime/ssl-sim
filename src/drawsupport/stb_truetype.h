...
// [ImGui] we added stbtt_PackFontRangesGatherRects() and
// stbtt_PackFontRangesRenderIntoRects() and modified stbtt_PackBegin()
...
#define STBTT_sort(data, num_items, item_size, compare_func)                   \
  qsort(data, num_items, item_size, compare_func)
#endif
...
static stbtt__active_edge *new_active(stbtt__edge *e, int off_x,
                                      float start_point, void *userdata) {
  stbtt__active_edge *z = (stbtt__active_edge *)STBTT_malloc(
      sizeof(*z), userdata); // @TODO: make a pool of these!!!
      ...
      if (w == 0) {
        ...
        if (i < len && j >= 0) {
          if (i == j) {
            ...
            scanline[i] =
                scanline[i] + (stbtt_uint8)((x1 - x0) * max_weight >> FIXSHIFT);
          } else {
            if (i >= 0) // add antialiasing for x0
              scanline[i] =
                  scanline[i] +
                  (stbtt_uint8)(((FIX - (x0 & FIXMASK)) * max_weight) >>
                                FIXSHIFT);
            else
              i = -1; // clip
...
STBTT_DEF int stbtt_PackFontRangesGatherRects(stbtt_pack_context *spc,
  ...
  for (i = 0; i < num_ranges; ++i) {
    ...
    for (j = 0; j < ranges[i].num_chars_in_range; ++j) {
      int x0, y0, x1, y1;
      int glyph =
          stbtt_FindGlyphIndex(info, ranges[i].first_unicode_char_in_range + j);
      if (glyph) {
